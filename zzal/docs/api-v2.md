...
- 답하면 대사 1줄 + 반응 동작 1개 + 친밀도 +40. `BABY`는 하루 3회에 **안 센다**(친밀도 +40과 2층 조건 카운터에는 센다, 정본 16장).
- **「해석」54 `chatReply.reactionKey`** — 답하기(2층 13번)가 열려 있으면 `reply`, 아직 잠겼으면 **`hello`**(정본 6장이 이름으로 못 박은 두 예외 중 하나: 답하기는 1층에 대응 행동이 없어 잠긴 동안 인사를 쓴다). 화면은 **받은 키를 그대로 재생**하므로 서버가 `pet`을 주면 화면의 대체표를 거치지 못한다. 키는 **이번 답을 센 뒤에** 고른다 — 그래서 **해금되는 바로 그 네 번째 답부터 `reply`**다(2026-09-22 개정, 옛 동작은 네 번째 답까지 `pet`이었다).
...
- 앨범은 **처음부터** 열려 있다(`features.album`은 **항상 true**). 기본 행동 8종부터 담기므로 심화 행동을 기다리지 않는다(2026-09-22 개정 — 옛 값은 "첫 심화가 도착해야 true"였고, 화면이 그 플래그로 입구를 막지 않아 계약만 정본과 어긋나 있었다). 조회(`GET /album`)도 막지 않는다.
- **아침 공개**(정본 2·16장) — 검수를 통과한(`OPEN`) 심화 행동은 **그 판정 뒤에 처음 맞는 기상**에 도착하고 그때 `revealedAt`이 찍힌다. 자는 동안에는 안 오고, 자는 펫을 깨우면 **깨우기(`POST /wake`) 응답 그 자리에서** 온다.
  - **「해석」55 판정이 낮에 끝나도 그날은 안 준다**(2026-09-22 개정). 마지막 기상 뒤에 판정된 것은 **다음 기상까지 보류**한다 — "자는 동안 연습해서 아침에 배워 왔다"는 이야기를 지키기 위해서다. 옛 동작은 "깨어 있는 첫 조회"라 낮에 판정되면 그날 낮에 줬다(해석 31의 옛 문장 폐기). 기준 시각은 굽기가 끝난 때가 아니라 **사람 판정이 끝난 때**이며, "기상"은 **밤잠에서 깬 것**만이다(낮잠은 아니다).
  - 판정 시각이 없는 옛 행은 가두지 않고 그대로 도착시킨다. **2층 기본 행동 해금은 이 규칙과 무관하다** — 조건을 채운 그 자리에서 즉시 열리고 `justUnlocked`에 실린다.
...
- **「해석」27 이어치기는 없다**(2026-09-22 개정, 옛 "같은 날이면 이어 친다"는 폐기 — 정본에 규정이 없던 빈자리였다). `start`는 **미완료 판이 남아 있으면 그 자리에서 패로 접고** 새 판을 연다. 나갔다 온 사람도 새 판이라 **하루 3판이 정상 차감된다**. `GET /current`는 그래서 판을 돌려주지 않는다(`playing`은 항상 false) — 돌려주면 그 `gameId`로 `guess`를 계속 쳐 규칙이 뒷문으로 무력화된다. 주소는 남겨 두었고(없애면 화면이 404를 받는다) 응답의 `remainingToday`·`runUnlocked`는 최신 값이다.
  - **거절되는 요청은 남아 있던 판을 접지 않는다** — 아픔·달리기 잠김·하루 한도 검사를 먼저 통과해야 접는다. 치던 중에 잠긴 달리기 버튼을 눌렀다고 판이 죽으면 안 된다.
...
- **「해석」56 조각과 2층 15번은 "끝까지 친 매치"로 센다**(2026-09-22 개정, 옛 "시작한 매치" 폐기). 좌우 맞히기는 **승패가 갈려 판이 끝난 순간**(3승 또는 3패 — 해석 57), 달리기는 **`finish`를 받은 순간** 세고 **승패는 안 본다**. 기권·강제 종료(다음 `start`가 접은 판)한 매치는 어느 쪽도 세지 않는다 — 시작으로 세던 때는 시작·나가기를 되풀이해 조각을 채우고 동작을 열 수 있었다. **하루 3판 차감만 시작 기준**으로 남는다(지는 판을 버리고 다시 시작하는 것을 막는 것이 그 상한의 목적이므로).
...
| 호출 | 요청 | 비고 | 거절 |
|---|---|---|---|
| `POST /{id}/call-back` | | 여행 중 "부르기" → 즉시 귀환·엽서 전달·친밀도 최고치 50%·케어 미스 0·게이지 전부 2. 응답 = `PetDetail` | `ZZAL_NOT_TRAVELING` `ZZAL_PET_NOT_ALIVE` |
| `POST /{id}/settings` | `{leaveEnabled}` | 떠남 끄기. 끄면 예고 중이던 짐 가방도 즉시 사라진다(여행 중인 아이가 돌아오지는 않는다 — 부르기로) | |
...
- **한 아이에 하루 20줄**까지. 넘으면 409 이고, 그 "하루"는 **취침 기준**이다(2026-09-22 개정, 옛 한국 시각 자정은 폐기). 다른 상한(놀이 3판·간식·쓰다듬·목욕)과 같은 경계다 — **마지막 밤잠 기상부터 다음 기상까지**이고, 자정을 넘겨도 잠들기 전이면 같은 하루다. 낮잠은 경계가 아니다. 경계가 갈려 있던 때는 자정을 낀 늦은 밤에 **한 번 깨어 있는 사이에 상한이 두 번 풀렸다**. 막으려는 것은 사용자가 아니라 눌린 채 굴러가는 화면이라 상한이 넉넉하고, 조용히 버리지 않고 분명히 거절한다.
...
```jsonc
{
  ...
  "pieces": null,                        // 3층 전: null. 3층: { "food", "play", "clean", "bond", "count", "streak", "bonus" }
  "goodDay": false,                      // 오늘이 "기분 좋은 날" 인가 — 조각 하나 선지급 + 살가운 첫 부름
...
| 15 | `startle` | 놀라기 | BASIC_2 | 미니게임 4판(승패·종류 무관, **끝까지 친 매치** 기준) | `놀라기` |
...
- **「해석」** `progress`는 2층 잠긴 칸에만. 조건 종류별 `current` = 밥 준 수 / 간식 수(그날 4개까지) / 청소 수 / 목욕 수 / 채팅 응답 수 / 쓰다듬기 수 / **끝까지 친 미니게임 매치 수**(2026-09-22 개정) / 손으로 깨운 밤잠 수.
...
| **`ZZAL_SICK_REFUSES`** | 409 | 아픈데 간식·게임(게임은 `start`·`guess`·`finish` 전부. `abandon`만 허용) |
...
| **`ZZAL_MOTION_WISH_DAILY_LIMIT`** | 409 | 한 아이에 동작 요청을 하루 20줄 넘게 남김(**기상에 초기화** — 취침 기준, 2026-09-22 개정) |
...
| 25 | 1.6 | **앨범은 잠그지 않는다** — `features.album`은 **항상 true**이고 `GET /album`도 항상 200(정본 6·16장 "앨범 = 처음부터"). 잠긴 칸의 이름·조건을 보여주는 것이 정본이므로 잠글 자리가 애초에 없다. **(2026-09-22 개정 — "첫 심화가 도착해야 true"라던 옛 문장은 폐기)** |
...
| 31 | 1.6 | 아침 공개는 시각이 아니라 **판정 뒤 처음 맞는 기상**에서. 자는 동안은 안 오고, 깨우면 그 응답에 실린다(정본 2·16장). **(2026-09-22 개정 — "깨어 있는 첫 조회 / 10:00을 넘겨 판정되면 낮에 온다"라던 옛 문장은 폐기. 지금은 낮에 판정돼도 다음 기상까지 기다린다 → 해석 55)** |
...
| 46 | 2 | 밤 연습 장면은 **잠들 때 남긴 쪽지**로 만들고, **아픔·기분도 잠들 때의 것**을 함께 적어 그것으로 판정한다(밤새 병이 나도 그 밤 장면은 남고, 아침에 나아도 아팠던 밤엔 안 남는다). — 밤새 앱을 안 열고 아침에 열어도 온다(정산이 잠·기상을 한꺼번에 처리하므로 "지금 자고 있나" 로는 못 잡는다). 잠든 그 시각으로 한 컷(같은 잠에 두 번 없음). **아프면 안 남기고**, 기능이 열리기 전(첫 부재 4시간 전)에도 안 남긴다 |
| 47 | 2 | **여행 중에는 장면이 없다**(방에 없으므로 — 그때 이야기는 엽서가 맡는다). 소품은 하루에 하나(날짜가 씨앗) |
| 48 | 2 | **조각 판정 시점**(2026-09-22 개정 — "판정은 잠들 때만 / 밥 2회·게임 1승·청소 1회·목욕 1회·채팅 1회·쓰다듬 2회"라던 옛 문장은 폐기. 정본 §16과 서버 실물에 맞춘다). 네 칸의 요구량은 **밥 6회 / 간식 5개·게임 5매치(어느 길이든) / 청소 5회·목욕 2회 / 채팅 5회·쓰다듬 5회**(`ZzalRules.PIECE_*`)이고, 규칙은 다섯 줄이다.<br>① **행동한 그 순간 센다** — 잠들 때 모아 판정하지 않는다.<br>② **하루로 끊지 않고 쌓인다** — 요구량 자체가 이틀치라 날짜가 넘어가도 누적이고, 연속을 세지 않는다(못 온 날에 벌을 주지 않으려는 것). 요구량에 닿는 순간 도장이 찍히고 **넘친 만큼은 버린다**.<br>③ **3층 전에는 아예 안 센다** — 조각 칸이 화면에 없는데 뒤에서 쌓이면 3층이 열린 날 한 칸이 공짜로 차 있다. 그 전의 돌보기는 소급하지 않는다.<br>④ **네 칸이 다 차면 그 자리에서 굽기가 시작되고**, 그날은 네 칸이 찍힌 채로 보인다. **다음 기상**에 네 칸이 0으로 돌아간다(그날의 성취를 보여 주고 다음 날 새 판을 시작하게 한다).<br>⑤ **기권·강제 종료한 매치는 놀이 칸에 안 센다**(해석 56) — 놀이 칸은 **끝까지 친 매치**와 간식으로만 찬다. 배탈이 난 간식(그날 5개째)도 안 센다. 쓰다듬기는 하루 3회까지만 센다(거절이 없어 연타로 채울 수 있으므로) |
| 49 | 2 | 조각 4칸은 2층 8종을 다 연 **그 뒤에 맞는 기상**에 등장한다(같은 날 저녁엔 아직). 서버가 "다 열린 시각"과 "열어 준 시각"을 따로 기억하고, **"다 열린 시각"은 관측 시각이 아니라 마지막 칸이 실제로 열린 시각**이다 — 밤에 완성되면 **바로 다음 날 아침**에 등장한다(하루 늦지 않는다) |
| 50 | 2 | 3층 심화 순서 = **13장 번호 순**(선물 101·102는 순서 밖). 지시문(`advanced-motions`)이 없는 동작은 건너뛴다 |
| 51 | 2 | **굽기에 실패해도 조각을 다시 모을 필요는 없다** — 큐에 올릴 때 연속을 소모하지만, 실패한 행은 `FAILED`로 다음 밤에 자동으로 다시 오른다(정본 16장 "조각을 소모하지 않는다") |
| 52 | 2 | 기분 좋은 날 = 잠들 때 그날 케어 미스 0 + 세 게이지 2칸 이상 → **다음 날 기상에** 조각 1개 선지급(`pieces.bonus`) + `goodDay: true`. 잠들면 꺼진다 |
| 53 | 5 | 두 번째 선물(`fall_back`)은 **튜토리얼을 끝낸 뒤 좌우 맞히기를 완주하고 못 이긴 그 순간** 큐에 오른다(2026-09-22 개정 — "3층 심화 8종이 열린 뒤"라던 옛 문장은 폐기, 그 값을 담고 있던 상수도 제거). 한 라운드 패배는 패배가 아니고(한 매치가 곧 5라운드 3선승), **기권·강제 종료·달리기 패배는 세지 않는다** — 한 판도 끝까지 안 친 사람이 선물을 받으면 그 이유를 모른다 |
| 54 | 1.8 | 예고는 **기상 순간**에만 판정한다 — 조회 때마다 보면 "5일째"가 사람마다 다른 순간이 된다. 앱을 안 열어도 밤 스위프의 정산이 그 경계를 지나므로 **안 오는 사람에게도 똑같이** 돈다 |
| 55 | 2 | 예고가 접속으로 취소돼도 **그 응답 한 번은** `leaving{justCancelled:true}`로 알려 준다 — 짐 가방이 그냥 사라지면 사용자는 자기가 무엇을 막았는지 모른다. ★ **`noticedAt`이 살아 있는 응답은 사실상 나오지 않는다**(조회 자체가 "접속"이라 그 자리에서 취소되므로) — 화면의 짐 가방은 `justCancelled`로 한 번 보여주면 된다 |
| 56 | 2 | 30일 2배는 **예고 일수와 유예 일수 둘 다**(5→10일, 2→4일). 케어 미스 8 쪽은 2배가 아니다 — 그건 날짜가 아니라 방치의 양이다 |
| 57 | 1.8 | 엽서는 **여행 중에는 안 보인다**(재회 때 전달). 여행 중에 다 보여 주면 부르러 갈 이유가 없어진다. 내용은 레시피(어디서·언제·몇 번째)이고 문구는 저장하지 않는다. ★ **밀린 몫은 한꺼번에 채워진다** — 출발한 날부터 지난 날수로 몇 장이 왔어야 하는지를 계산하므로, 여행 중 앱을 한 번도 안 열고 곧바로 불러도 세 장을 받는다(조회 여부가 결과를 바꾸지 않는다). 각 엽서의 시각은 **그 날짜**다 |
| 59 | 1.8 | **여행 중에는 밤 큐가 아무것도 안 굽는다**(재등록 포함) — 집을 비운 사람의 몫이 매일 밤 API 로 구워지면 안 되고, 게이지·조각이 다 멈춘 채 굽기만 도는 것도 앞뒤가 안 맞는다 |
| 58 | 1.8 | 여행 중에도 **설정 변경은 된다**(방 밖 이야기). 다만 떠남을 꺼도 이미 떠난 아이가 돌아오지는 않는다 — 부르기로 데려온다 |
...
- **2026-09-22** — 연결 감사(정본 v1.11 대조) 결정 반영: 앨범 플래그 상시 true(해석 25) · 이어치기 폐지와 `current` 무력화(해석 27) · 조각·2층 15번을 완주 매치 기준으로(해석 56) · 매치는 3승 또는 3패에서 끝남을 명시(해석 57) · 심화 공개를 다음 기상까지 보류(해석 55) · 채팅 반응 키 `hello`/`reply`와 고르는 시점(해석 54) · 두 번째 선물 조건 정정(해석 53) · 동작 요청 하루 경계를 취침 기준으로(1.9) · 병 중 게임 전면 거부 명시.
...
- **2026-09-05** — PR-11(#136) 리뷰 반영(#235): 엽서 밀린 몫 backfill, 여행 중 밤 큐 정지, 엽서 정렬을 쓴 시각으로. 해석 57 보강·59 추가.
- **2026-09-05** — PR-11(#136): 떠남·재회(9절), 해석 54~58.
- **2026-09-05** — PR-10(#205) 리뷰 반영(#234): 조각 등장이 한 기상 늦던 것(완성 시각 = 해금 시각), 밤 장면 판정을 잠들 때 상태로, 두 번째 선물 가드. 해석 46·49 정정.
- **2026-09-05** — PR-10(#205): 조각과 3층 심화 순서(9절), 해석 48~53.
