...
- **「해석」** 아기 60분 안에 19~23시가 겹치면 `sleep`은 **낮잠이 우선**(낮잠을 이미 썼으면 밤잠).
...
| 16 | 1.3 | 낮잠은 재우기·깨우기 둘 다 보상 0(밤잠만 +1·+10) |
| 17 | 1.3 | 아기 60분 안 19~23시 재우기는 낮잠 우선(낮잠 뒤엔 밤잠) |
| 18 | 1.3 | 낮잠에서 깬 순간이 밤이면 그 응답 안에서 밤잠 전이 |
| 19 | 1.3 | 자동 경계에서 열리는 해금(15번)은 `justUnlocked` 대신 아침 `learnedToday` 계열(PR-4 이후) |
...
- **2026-09-05** — 리뷰 반영(PR-2): 해석 16~20 추가(낮잠 보상 0·낮잠 우선·깬 직후 밤잠 전이·자동 경계 해금 알림·비-ALIVE 리스트 `[]`).
