...
public enum PieceEvent {
    ...
    /**
     * 미니게임 한 판을 <b>끝까지 쳤다</b>(매치 기준, 승패 무관).
     *
     * ★ 시작이 아니라 <b>완주</b>다(정본 7-A) — 기권·강제 종료한 매치는 이 자리에 오지 않는다.
     *   시작으로 세던 때는 시작만 하고 나가기를 되풀이해 조각을 채울 수 있었다(연결 감사 F7).
     * ★ 2층 15번(놀람)의 "게임 4판" 도 같은 선이다({@code ZzalPet.finishGame()}) — 같은 자리에서
     *   함께 센다. 둘이 갈리면 "조각은 안 차는데 해금은 되는" 상태가 생긴다.
     */
    GAME(PieceKind.PLAY),
    /** 청소했다 */
