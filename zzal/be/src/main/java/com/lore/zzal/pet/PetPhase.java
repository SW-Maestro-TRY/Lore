...
public enum PetPhase {
    ...
    /**
     * 초안. 그림만 받았고 <b>이름이 아직 없다</b>. 생성은 <b>이미 돌고 있다</b> — 시트부터
     * 격자·후처리까지, 그림을 올리는 순간 전부 시작한다.
     *
     * ★ 왜 이 단계가 따로 있나 — 부화 전체가 2~7분이라, 사용자가 이름을 짓는 동안(약 74초)
     *   생성이 함께 돌아 그만큼 앞당긴다. 이름이 들어오면 HATCHING 으로 넘어간다.
     *
     * ★ 이름을 안 짓고 나갔다가 다시 오면 이 초안을 이어간다 — 이미 구운 것을 재사용하므로
     *   돈이 두 번 나가지 않는다.
     */
    DRAFT,

    /**
     * 알. 이름까지 받았다. 생성이 아직 안 끝났을 뿐 <b>여기서 새로 시작하는 것은 없다</b> —
     * 이름이 들어온 시점에 생성이 이미 끝나 있었다면 곧바로 ALIVE 로 간다.
     */
    HATCHING,
