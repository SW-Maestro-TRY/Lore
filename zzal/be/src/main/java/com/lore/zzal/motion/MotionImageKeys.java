 ...
 * {@code 판 <= 0} 이면 <b>판 칸을 아예 넣지 않는다.</b> 값이 0 인 행이 섞였을 때
 * {@code .../basic/0/base.webp} 같은 <b>있지도 않은 주소</b>를 만들지 않기 위해서다.
 * 새 펫은 첫 판이 1이라 이 갈래를 타지 않는다.
 *
 * <p>★ 다만 <b>펫 상세 응답은 이 갈래의 주소를 더 이상 내려보내지 않는다</b> —
 * {@code PetResponses.Detail.basicImageKey} 가 판이 0 이면 {@code null} 을 준다. 판 0 은
 * "옛 주소에 파일이 있는 옛 펫" 과 "한 장도 굽지 않은 펫" 을 한꺼번에 뜻하는데, 후자에 주소를 주면
 * 화면이 대체 그림을 띄울 기회를 놓치기 때문이다(전제·대가는 그쪽 주석에). 여기 조립기는
 * 후처리 업로드 자리({@code PostProcessStep})와 공유가 여전히 쓰므로 모양을 그대로 둔다.
 *
