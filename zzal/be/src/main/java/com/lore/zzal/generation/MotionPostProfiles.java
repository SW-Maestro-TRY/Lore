 ...
 * <pre>
 *   구르기      발이 공중에 뜨는 칸이 있지만 확정본은 발 정렬(state16_v2)로 나왔다
 *   뒤로넘어짐  뒤로 눕는 칸에서 발 기준이 통째로 오판된다. 엉덩이·등이 닿는 자리
 *               (접지앵커)를 기준으로 맞춘다(state16_v3 · align=seat)
 * </pre>
 * 칸 번호·기준을 코드에 박지 않는다는 원칙의 16프레임 판이다 — 코드가 아니라 표를 고친다.
 *
 * <h3>★★ 없으면 굽지 않는다 — 기본값으로 떨어지지 않는다</h3>
 * 기본 프로파일을 두면, 새 동작을 확정하고 이 표에 줄을 <b>깜빡했을 때</b> 검수를 거치지 않은 후처리로
        ...
        if (profile == null || profile.isBlank()) {
            throw new IllegalStateException(
                    ("'%s' 의 후처리 프로파일이 없습니다. %s 에 `%s = script=..., ...` 줄을 적으세요 "
                            + "(적힌 동작: %s). 기본값으로 굽지 않습니다 — 검수를 거치지 않은 후처리로 구워진 "
                            + "그림은 화면을 봐야만 드러납니다")
