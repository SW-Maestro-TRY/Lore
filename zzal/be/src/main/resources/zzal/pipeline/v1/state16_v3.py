#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""16프레임 격자 후처리 v3 — 2026-09-12 신설. 상훈님 승인 후 착수.

`state16_v2.py`(v02·v4·v5 판정본을 만든 도구)는 **한 글자도 고치지 않는다**
(→ [[judged-artifacts-are-frozen]]). 이 파일은 새로 만들고, v6부터 쓴다.

왜 새 파일인가 — 2026-09-12 v5 실측 (근거.md 11-2)
---------------------------------------------------
상훈님: *"중간에 전체적으로 앵커가 한번 바뀌는 부분이 있는 것 같은데 확인해봐"*
...
2) **`--align foot` 이 딱 한 곳에서 폭발한다.** 소닉 f08→f09 에서 -48.5px.
   ...
   (→ [[umzzal-postprocess-by-posture]] 의 16프레임판. 8종 쪽은 `state8_v5` 가 이미 자세로 갈랐다.)
...
1. **절단 기준 = 검출된 마크선**(`--cut marks`, 기본). 25개 마크를 5열/5행으로 뭉쳐
   ...
   그 사실을 로그에 남긴다**(→ [[silent-config-mismatch]] · [[llm-judges-drift]]:
   판정은 코드가 결정적으로, 실패는 소리를 내야 한다).
...
출력 파일 이름·위치는 v1·v2 와 완전히 동일하다(러너·judge_add.py 가 그 이름을 찾는다):
...
def main(grid, cols=4, rows=4, cut="marks", align="posture", lie_from=9, lie_dx="fixed",
    ...
    # ── ★--align seat (2026-09-12 상훈님 판정 "앵커가 좀 흔들리네" 처방) ─────────────
