...
(`judged-artifacts-are-frozen` — 판정받은 도구는 안 고치고, 새 파일을 v02 로 만들어 다시 판정받는다).
...
3. **정렬을 선택적으로 만들었다 — 기본값은 `--align none`(정렬 안 함), `--align foot`을 주면
   ...
   ★기본을 none 으로 둔 이유 — 2026-08-18 판정에서 상훈님이 넘어지기류 재절단본을 보고
   *"마지막에 땅이 엄청 위로 우뚝 솟아"* 라고 지적하셨다. 선물 동작(구르기·뒤로넘어짐)은
   캐릭터가 눕거나 구르는 프레임이 많아서 `foot_ref`(실루엣 최하단 4% 밴드)가 매 프레임 다른
   신체 부위(등·엉덩이·머리)를 가리키게 되고, "16칸을 발 좌표 중앙값에 맞춘다"는 v1 의 전제 자체가
   깨진다. 그 상태에서 정렬을 강행하면 바닥선이 프레임마다 널뛰는데, 상훈님이 본 사고가 바로 그것
   이었다(state8_v5 가 서 있음/앉음/눕기로 자세를 갈라 대응한 것과 같은 문제의 16f 버전).
   16프레임에는 아직 state8_v5 같은 자세별 프로파일이 없으므로, **안전한 쪽(칸을 뜬 그대로 안
   건드림)을 기본으로 두고**, 서 있는 동작처럼 발이 실제로 고정인 시퀀스에서만 상훈님이 판단해서
   `--align foot`으로 켠다.
4. **정렬 여부와 무관하게 칸별 통계(발 좌표 산포·격자점 제거 px·침범 제거 px)를 항상 출력한다** —
   정렬을 끈 채로도 "이 시퀀스가 발 기준 정렬에 적합한 동작인가"를 상훈님이 숫자로 먼저 볼 수 있게.

출력 파일 이름·위치는 v1(`state16_post.py`)과 완전히 동일하다(러너·judge_add.py가 그 이름을 찾는다):
