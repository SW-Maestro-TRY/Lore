...
v3 대비 바뀐 것 다섯 (근거 = 조사 리포트 C_후처리_게이트_생성도구_이력.md)
--------------------------------------------------------------------
1. **격자점 제거를 hue 기반으로 + 위치 기준을 '실제로 찍힌 마크'로**(C 1-1).
...
2. **정렬 전에 캔버스를 먼저 넓힌다**(C 1-8·6절, `루프빌드.py` PAD=60 방식).
...
5. **칸을 위로도 넓게 뜬다 — 아래 PAD 와 대칭**(2026-09-12 상훈님 승인).
...
- 로그 라벨을 v3 이름(기본·식사·기쁨·슬픔·아픔·교감·인사·자기)으로 바로잡았다.
  ...
  "훈련이 이상하다"고 잘못 결론내게 된다** — 실제로는 눕기다(C 6절).
- 층2 보정이 |dx|>30 또는 |dy|>40 이면 스스로 경고를 찍는다(C 4절 5번).
...
6. **눕기·웅크림 쌍의 층2 가로를 칸 중앙에 맞춘다**(2026-09-12 추가, 기본 켬 · `--no-center-lying` 로 끔).
   ...
   **13~35px 왼쪽**에 앉았다(독립검증 1-4절). 여백은 0 을 넘어 통과하지만 8종을 이어 보면
   잘 때만 옆으로 미끄러져 보인다. → 본체 bbox 가로 중심을 **칸 중앙(156)** 에 맞춘다.
   발 중앙값(rx)이 아니라 칸 중앙을 쓰는 이유 = 누운 몸엔 발 기준이 없고, rx 는 판마다 흔들린다.

7. **눕기·웅크림 쌍의 층1 탐색에 '시드'를 준다**(2026-09-12 추가 · 상훈님 판정에서 잡힘).
   상훈님이 v01 판정 중 *"블룸이 쪼그려 앉는 자세에서 좌우좌우 왔다갔다"* 를 잡으셨다.
...
8. **후처리를 '하나'로 두지 않고 자세 유형별 프로파일로 가른다**(2026-09-12 상훈님 지시).
   *"동작별로 후처리를 좀 따로 둬도 돼. 발 닫고 있는 동작들용 후처리라던가 앉고있는 거,
     눕고 있는 거용 후처리라던가 이런 식으로"*
...
# ── 자세 유형 프로파일 ───────────────────────────────────────────────────────
# ★2026-09-12 상훈님 지시: *"동작별로 후처리를 좀 따로 둬도 돼. 발 닫고 있는 동작들용 후처리라던가
#   앉고있는 거, 눕고 있는 거용 후처리라던가 이런 식으로"*
...
PAD_CUT_TOP = 60  # ★칸 **위**에도 아래와 대칭으로 여유를 둔다 (2026-09-12 상훈님 승인).
...
def best_seeded(a, b, rng=SEED_RNG):
    ...
    ★왜 (2026-09-12 상훈님 판정 "블룸이 쪼그려 앉는 자세에서 좌우좌우 왔다갔다")
...
def parse_postures(spec, base=None):
    ...
    ★왜 인자인가 (2026-09-12 상훈님 지시) — 2층·심화에서 앉기·눕기가 다른 칸에 오면
