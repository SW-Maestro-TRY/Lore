...
2026-08-06 착안 — 땅과 마크를 아예 고정축 프레임으로 삼는다.
...
def find_marks_by_color(im: np.ndarray):
    ...
    왜 바꿨나
      ...
      → 프롬프트에서 색을 못 박았다: **좌=마젠타(#FF00FF) · 우=시안(#00FFFF)**.
...
def find_all_marks(im: np.ndarray, bg: np.ndarray) -> list:
    ...
    ★2026-08-09 크기·픽셀수만 보던 기준에 **모양과 위치**를 더했다.
      ...
      ⚠️근본 해법은 격자 생성 프롬프트에서 마크 색을 못 박는 것이다. 그때까진 이 필터.
    """
...
def find_ground(d: np.ndarray, x0: float, x1: float, y_from: int, y_to: int) -> float:
    ...
    ★세로 기준은 마크가 아니라 땅이어야 한다(2026-08-06 검수):
      칸 안에서 땅의 위치가 오르내려 앵커의 상하 움직임이 너무 커 보기 어려웠다.
    ...
    ★2026-08-16 — **실패를 숨기지 않게 고쳤다.**
      ...
      마크+98을 땅으로 반환했고, 그 결과 프레임이 세로로 튀어 검수에서 '대실패'로 떨어졌다.
...
def clean_margin_bands(arr, bg, top_y, bot_y):
    ...
    왜 (2026-08-14 검수)
    --------------------
    여백을 20→40으로 키우니 잘림이 거의 사라졌다 — 발도 머리도 다 살아났다.
    남은 결함은 두 건이었다 —

      · 블룸 애교 v04 : 후반부 프레임에서 머리 위에 발인지 땅인지가 조금 보인다  (위 칸 침범)
      · 여울 콩콩뛰기 v10 : 발 아래에 안테나가 살짝 보인다                      (아래 칸 침범)
...
def main() -> None:
    ...
    ap.add_argument("--margin", type=int, default=40,
                    help="프레임 바깥으로 남길 여백. ★상하좌우 모두 같은 값이다. "
                         "★2026-08-14 기본값 20→40 (검수에서 발·머리 잘림이 사라진 값). "
                         "기존 137편은 재절단하지 않는다 — "
                         "앞으로 만드는 것에만 적용")
    ...
    if color_mode:
        ...
        if not args.allow_broken:
            raise SystemExit(
                ...
                "  깨진 격자를 잘라 검수까지 올리면 동작이 아니라 후처리를 보게 됩니다"
    ...
    #   예전에는 균등분할로 대체했는데, 그건 8/6에 이미 폐기한 가정이다
    #   (실측: 모델이 그리는 격자는 균등분할이 아니라 줄 23px·열 14px 어긋난다).
    #   그래서 폴백이 걸린 칸만 캐릭터가 작아지고 위치가 틀어졌고, 마크를 못 찾았으니
    #   **지우지도 못해 십자가 그대로 남았다**(2026-08-09 블룸 손흔들기 판3 검수:
    #   앵커가 크게 흔들리고 마크가 안 지워진 채로 올라왔다 — f02·f09·f11).
    ...
    # ★출력 크기를 프레임에서 정한다(2026-08-06 — 프레임 상하좌우의 크기를 같게).
    ...
    for i, p in enumerate(pairs, 1):
        ...
        #   (9개 동작에서 앵커가 좌우로 흔들린다는 지적이 나왔다). 중점을 쓰면 좌우 오차가 서로 상쇄된다.
        ...
        # ★★fillcolor를 배경색으로 준다 (2026-08-11, 네 번 지적된 'gif 하단이 왔다갔다'의 진범).
        #   프레임이 격자 이미지 밖을 참조하면 PIL은 기본값 **검정(0,0,0)**으로 채운다.
        #   마지막 행(f13~16)은 격자 아래가 없으므로 하단에 검은 띠가 생기고, 그 높이가 칸마다 달라
        #   눈에는 '하단 길이가 왔다갔다'로 보인다(여울 박수 v13 실측: f1~12 배경색 249,245,239 /
        ...
        # ★프레임 밖은 배경으로 덮는다(2026-08-06 검수):
        #   바닥 아래로 안테나가 보이거나 gif 하단의 길이가 들쭉날쭉해 보였다.
        #   땅 아래를 원본에서 그대로 가져오면 아랫칸 캐릭터의 안테나가 딸려오고,
        #   칸마다 그 양이 달라 하단 길이가 들쭉날쭉해진다. 프레임 밖은 그림이 아니다.
        arr[int(round(y_ground)) + m:, :] = bg
        # ★좌우도 덮는다(2026-08-06 검수 — 왼쪽에 검은 것이 왔다 갔다 해 보였다).
        ...
        if args.clean_bands:
            ...
            # ⚠️+1로 잡았더니 바닥선 아래 절반이 청소 구간에 들어가
            #   땅 색깔이 바뀌어 보였다. 바닥선 두께를 프롬프트에서
