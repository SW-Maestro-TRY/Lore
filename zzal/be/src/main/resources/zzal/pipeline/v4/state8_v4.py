...
무엇이 문제였나 (2026-09-02 검수)
----------------------------------
검수에 올린 5캐릭터와 서비스가 만든 민트 8종 **모서리에 자주색 조각이 남아 있다.**
원인은 모델이 그리는 마크 색이 우리가 정한 색과 정확히 같지 않다는 것이다.
...
같은 기준을 여기 그대로 가져온다. 마젠타 hue 280~340 · 시안 hue 165~205.
...
v3 의 초록 키잉·despill 로직(검수 537건이 걸려 있다)은 한 줄도 건드리지 않는다.
