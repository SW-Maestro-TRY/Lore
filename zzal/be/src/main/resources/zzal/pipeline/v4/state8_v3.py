...
NAMES=["기본","식사","배고픔","청소","행복","불행","쓰다듬","훈련","잠"]  # 2026-08-25 재구성(졸림→청소, 9번째 잠은 6x3 시험용)

def key_green(cell, cell_h=None, lo=10, hi=90, despill=True):
    """초록 배경을 지운다 — **알파는 연속값**(2026-08-25 전면 개편).

    ★왜 바꿨나 (2026-08-25 검수 — 실루엣 가장자리가 벌레 먹은 듯 울퉁불퉁했다.
      한 캐릭터만의 문제가 아니었다)
    ...
    #   검수에서 드러난 결함: 김애용·소닉의 **초록 눈**(RGB 57,137,48 → 우세도 80)이
    ...
    if n_bg:
        ...
        #   초록 얼룩이 남았다 — 사고다. 셋을 모두 만족할 때만 캐릭터로 인정한다.
    ...
    #   검수 지적: 김애용 훈련 칸의 땀방울 RGB(76,221,235)가 옛 조건
...
def foot_ref(im, frac=0.04):
    ...
    ⚠️frac 은 0.04(4%)다. **0.12로 잡았다가 틀렸다**(2026-08-24 검수 — 흑연 8번째 칸의 발이 눈에 띄게 움직였다).
    ...
    ★왜 (2026-08-24 검수 — 앵커가 너무 많이 흔들린다)
      겹침(IoU) 탐색은 실루엣 전체를 맞추므로, **커지는 기호와 퍼지는 머리카락**에 끌려다닌다.
      검수에서 짚힌 흔들리는 칸(배고픔·쓰다듬·졸림)이 전부 '공중에 뜬 기호가 커지는 상태'였고,
    ...
    ★꼬리는 발이 아니다 (2026-08-25 검수 — 1프레임에는 꼬리가 있고 2프레임에는 없어
      발이 제대로 잡히지 않았고, 맞춘 뒤에도 발이 흔들렸다)
...
def mk_char(im):
    ...
    ★왜 (2026-08-25 검수 — 좌우 앵커가 크게 흔들린다)
...
def main(grid, cols=4, rows=4):
    ...
    ★cols/rows 를 인자로 뺐다 (2026-08-25) — 4x4 말고 다른 형식도 되는지 보려고
      9종 18칸(6열x3행)을 시험했다. 기본값은 4x4 그대로라 기존 호출은 영향이 없다.
    """
    grid=Path(grid); R=grid.parent
    npairs = cols*rows//2
    im=Image.open(grid).convert("RGB"); W,H=im.size; cw,ch=W/cols,H/rows
    # ★칸 아래로 여유를 두고 자른다 (2026-08-24 — 마커보다 조금만 더 아래까지 자른다)
