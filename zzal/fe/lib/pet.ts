...
export interface Pieces {
   ...
   *
   * ★★ **서버가 안 준다**(2026-09-20 확인 — `PetResponses.Pieces` 에 이 칸이 없다. 1.8 에서
   *   "조건을 채운 그 순간 굽기" 로 바뀌며 연속일수가 규칙에서 빠졌다). 목 서버만 채운다.
   *   그래서 선택 항목이다 — 필수로 두면 타입은 통과하는데 **실행할 때만 `undefined`** 가 되고,
   *   그걸 읽는 화면은 조용히 아무것도 안 그린다(실제로 "N일 연속" 칩이 한 번도 안 떴다).
   *   되살리려면 서버 응답에 칸이 먼저 생겨야 한다.
   */
  streak?: number;
