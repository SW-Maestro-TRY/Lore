 ...
 * 성격 화면의 네 묶음.
 *
 * ★★ `ph`(자리표시글)는 **짧게 — 한 줄에 다 보이게**(2026-09-22 상훈님 "기존처럼 깔끔하게").
 *   예전 문장은 28~35자라 390 폭 입력칸에서 **뒤가 잘렸다**(잘린 안내는 안내가 아니라 소음이다).
 *   "적어 주세요" 같은 지시는 빼고 **예시 하나만** 둔다 — 무엇을 적는 칸인지는 제목이 이미 말한다.
 * ⚠️ 여기 **key 이름은 바꾸지 말 것** — 서버로 나가는 칸 이름이고 다른 화면도 같이 읽는다.
 */
export const CHAR_GROUPS = [
  { key: 'persona', label: '성격 · 여럿 고를 수 있어요', opts: PERSONA, ph: '예: 낯을 가리지만 잘 따라와요' },
  { key: 'tone', label: '말투', opts: TONE, ph: '예: 말끝을 길게 늘여요' },
  { key: 'genre', label: '장르', opts: GENRE, ph: '예: 잔잔한 학원 일상' },
  { key: 'world', label: '세계관', opts: WORLD, ph: '예: 현대 도시 · 기숙학교' },
] as const;
...
/**
 * ★★ 옛 `TUTOR_MAIN`(진짜 방 목 전용 8칸)은 **없앴다**(2026-09-22).
 *
 *   진짜 방의 튜토리얼은 **서버가 세는 아홉 칸 하나**뿐이다. 목이 여덟 칸짜리 다른 목록을
 *   들고 있어서, 3202(서버 없는 화면)에서는 **상훈님이 진짜 튜토리얼을 못 밟았다** —
 *   순서도 문구도 칸 수도 서버와 달랐다. 두 벌을 두면 한쪽만 고쳐지고, 갈린 쪽은 조용하다.
 *   → 아래 `TUTOR_ROOM` **한 벌을 서버와 목이 같이 쓴다.**
 */
...
/**
 * **진짜 방의 튜토리얼 아홉 칸 — 서버와 목이 같이 쓰는 한 벌**(2026-09-22).
 *
 * ★★ **두 벌로 두지 말 것.** 예전에는 서버용 9칸과 목용 8칸(`TUTOR_MAIN`)이 따로 있었다.
 *   그래서 **서버 없는 화면(3202)에서는 진짜 튜토리얼을 밟을 수 없었고**, 문구를 고쳐도
 *   한쪽만 고쳐졌다. 오늘 하루에만 같은 병이 세 번 났다(한도 숫자 여덟 곳 · 안내판 목 상수 ·
 *   원망 필터 두 벌). 목에서 밟은 것이 곧 서버에서 밟을 것이어야 연습이 거짓말이 아니다.
 *
 * 순서·개수는 서버 `TutorialStepKey` 와 **한 칸도 어긋나면 안 된다** — 서버가 준 `tutorial.step` 이
 * 곧 이 배열의 첨자다. 어긋나면 엉뚱한 방을 가리키고, 아무 소리도 안 난다.
 * 목은 같은 `done` 키로 제 칸을 넘긴다(→ `useYeoul.tutorDone`).
 *
 * 문구는 **정본 §12 표의 "아이가 부름" 아홉 줄**을 이 화면 말로 편 것이고,
 * 규칙 여섯 개(§12 둘째 표)는 **그 규칙을 쓰는 칸**에 한 줄씩 얹혀 있다.
 *
 ...
 */
export const TUTOR_ROOM: readonly TutorStep[] = [
  ...
  // ★ 정본 §12 는 이 칸을 **"이 성격이 맞나요"**(확인·수정)로 적는다 — 성격은 부화 전 캐릭터
  //   정보 화면에서 이미 받았으므로 여기서 처음 고르는 것이 아니다(정본 1.3·1.6).
  { at: '4칸', room: 'info', act: null, done: 'PERSONALITY', text: '이 성격이 맞나요. 아이 정보에서 확인해 주세요.' },
 ...
 *
 * ★★ **2층만 적는다**(2026-09-22 판정 4). 예전에는 옛 목록이 남아 **이미 열려 있는 1층**
 *   (손 흔들며 인사 = `hello` · 자기 = `sleep`)이 "배울 것" 으로 올라와 있었다. 4차에 서버 쪽은
 *   도감으로 고쳤는데 **목 폴백만 옛 상수**였다. 이름은 앨범·서버와 같은 말(`POSE_LABEL`)로,
 *   조건·횟수는 **정본 §6 2층 조건표 그대로** 적는다.
 * ★ 목이 셀 수 있는 것만 올린다 — 나머지 넷(밥 먹기 9 · 간식 먹기 9 · 청소하기 13 ·
 *   쓰다듬받기 4)은 목에 계수기가 없다. **없는 숫자를 지어내는 것보다 안 적는 쪽**을 골랐다.
 *   서버에 붙으면 여덟 줄이 다 나온다.
 */
export const LEARN_GOALS = [
  { name: '답하기', cond: '대화 답하기', need: 4, counter: 'cChat' },
  { name: '목욕하기', cond: '목욕하기', need: 3, counter: 'cBath' },
  // ★ 정본 §6 16번 — **손으로 깨운 것만** 센다(아침 자동 기상·튜토리얼 낮잠 제외).
  { name: '일어나기', cond: '깨우기', need: 4, counter: 'cWake' },
  ...
  { name: '놀람', cond: '게임 끝까지 치기', need: 4, counter: 'cGame' },
] as const;
 ...
 *   사용자를 재촉하거나 원망하는 결은 쓰지 않는다.** 이 규칙은 그대로다.
 * ★★ 다만 **"지킬 수 없는 약속을 안 쓴다" 는 2026-09-22 에 상훈님이 한 번 접으셨다** —
 *   본문이 "자고 일어나면 보여드릴게요" 로 바뀌었다. 무엇을 알고 접었는지는 `body` 머리말에 적었다.
 *   다른 문구까지 이 예외를 따라가지 말 것. **이 한 줄에만 해당한다.**
 *
...
export const GRAD_COPY = {
  title: '첫날을 함께 마쳤어요',
  /**
   * ★★ **"배우는 중이에요" 와 "자고 일어나면" 두 마디를 지우지 말 것**(상훈님 결정 2026-09-22).
   *
   *   옛 문구는 *"…구르기를 하나 **그리는 중**이에요 / **다음에 오실 때쯤** 앨범에서 만날 수
   *   있을 거예요"* 였다. 여지를 남겨 **거짓이 될 수 없게** 만든 문장이었지만, 상훈님이 dev 에서
   *   읽고 **두 가지를 짚으셨다** — (1) '그리는' 은 만드는 사람의 말이지 **아이의 말이 아니다**,
   *   (2) '다음에 오실 때쯤' 은 여지라기보다 **언제인지 모르겠다는 말**로 읽힌다.
   *
   * ★★ **거짓이 될 수 있음을 알고 택한 문구다.** 구르기는 굽고 나서 검수를 통과해야 앨범에
   *   들어오고(`ZzalMotion.reveal` · `AdvancedMotion.imageKey` 는 도착 뒤에만 찬다), 그 검수가
   *   언제 끝나는지는 화면도 서버도 미리 모른다. 즉 **검수가 하룻밤을 넘기면 "자고 일어나면" 은
   *   그 사람에게 거짓이 된다.** 그 값을 알고도 고른 이유는, 첫날을 마친 사람에게 건네는 말이
   *   정확한 것보다 **아이의 말투로 들리는 것**이 더 중요하다고 판단하셨기 때문이다.
   *   ⚠️ 그러니 이 문구를 지키려면 **검수를 하룻밤 안에 끝내는 운영**이 함께 가야 한다.
   *   운영이 그걸 못 지키게 되면 문구를 되돌릴 것 — 주석만 남기고 문구를 두면 그때 또 거짓이 된다.
   *
   * ★ "여울처럼" 을 넣은 이유 — 이 자리 미리보기는 **여울의 구르기**(`GRAD_PREVIEW_SRC`)다.
   *   본문이 둘을 나란히 놓아야 사용자가 그림을 **제 아이의 것으로 오해하지 않는다**(자캐 규범).
   *   ⚠️ 다만 **연습방의 아이가 곧 여울**이라, 거기서는 "여울도 여울처럼" 이 된다. 그때는 뺀다 —
   *   같은 아이를 둘로 말하는 문장이 되고, 무엇보다 읽는 사람이 웃는다(2026-09-22 목에서 실측).
   *
   * @param name 아이 이름(빈 값이면 '아이').
   */
  body: (name: string) => `${name}도 ${name === '여울' ? '' : '여울처럼 '}구르기를 배우는 중이에요.\n자고 일어나면 보여드릴게요!!`,
 ...
 * ★ `limit` 문구의 "내일" 은 **여전히 참이다** — 다만 근거가 바뀌었다(2026-09-22). 하루 경계가
 *   **자정이 아니라 밤잠 드는 순간**으로 옮겨 간다(백엔드 작업 중 · `zzal/docs/api-v2.md`
 *   "하루의 경계 = 밤잠 드는 순간 … 자정은 아무 의미 없다"). 그래서 **시각을 약속하는 말은
 *   어디에도 쓰지 않는다** — "자정에 풀려요" 는 그 순간 거짓이 된다. "내일" 까지가 우리가
 *   지킬 수 있는 말이다.
