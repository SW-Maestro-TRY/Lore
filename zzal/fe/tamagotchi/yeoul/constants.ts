 ...
 * ★ "준비하고 있어요" 는 앨범 쪽지(`parts/AlbumNotes.tsx` 의 BAKING 줄)와 **같은 낱말**이다.
 *   같은 사건을 두 화면이 다른 말로 부르면 사용자가 둘을 다른 일로 읽는다.
 */
export const GRAD_COPY = {
  title: '첫날을 함께 마쳤어요',
  /** @param subj 이름+조사("여울이가"). */
  body: (subj: string) => `${subj} 둘러보는 법을 다 익혔어요.\n기념으로 구르기를 하나 준비하고 있어요. 다 되면 앨범에 담아 둘게요.`,
 ...
 * 그림 올리기 칸의 문구.
 *
 * ★★ `infraNote` 가 이 상수가 생긴 이유다. 연결이 끊기거나 S3 가 거절하면(CORS·403·5xx)
