 ...
 *     샘플 방 : 주방 204 · 욕실 204 · 마당 204 · 침실 129 · 앨범 186   ← 여울의 물음 카드가 얹혀 4px 더 높다
 *   **가장 큰 204** 를 쓴다. 이보다 낮게 잡으면 어느 방에선가 팝오버가 발을 덮는다.
 *
 * ⚠️ 팝오버(또는 그 위에 얹히는 물음·대화 카드)의 줄·여백이 바뀌면 이 값이 조용히 낡는다 —
 *   캐릭터가 가려지거나 붕 뜬다. 다시 재는 법: 방을 하나씩 열고 두 모드 모두에서
 *   `무대.getBoundingClientRect().bottom − 팝오버.top` 의 최댓값.
 */
