...
export function useYeoul(live?: Live) {
  ...
  const v = useMemo(() => {
    ...
    return {
       ...
       *
       * ★★ 2026-09-23 — `s.sampleMode` 조건을 **뺐다.** 2칸(`PET`)은 **가리킬 타일이 없는 칸**이다
       *   (`tut.room === null`). 아이는 단추가 아니라 그림이라 하이라이트도 `disabled` 도 못 붙고,
       *   그래서 진짜 방 2칸에서는 **켜진 손잡이가 화면에 하나도 안 보였다** — 물결이 연습방
       *   전용이었기 때문이다. 이 물결이 곧 그 칸의 하이라이트다.
       * ★ 남은 조건(`!tut || tut.act === 'pet'`)이 **다른 칸에서는 안 뜨게** 막는다 —
       *   튜토리얼 중에는 물결도 딱 그 칸에만 나온다. 튜토리얼이 끝난 방에서는 연습방과 같이
       *   "아이를 눌러 보세요" 가 계속 떠 있다(잠·아픔·창이 열린 동안에는 빠진다).
       */
      guide: {
        tap: s.screen === 'room' && !es.sleeping && !es.sick && !s.chatOpen && !s.sheet && !s.popOpen && !s.gOn && (!tut || tut.act === 'pet'),
        label: '톡 눌러 보세요',
      },
      sample: {
        show: s.sampleMode,
