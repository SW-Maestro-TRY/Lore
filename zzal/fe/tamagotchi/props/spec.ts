...
/** 소품이 붙는 자리. 앞 넷은 캐릭터를 따라가고, 뒤 셋은 무대에 고정이다. */
export type PropAnchor =
  ...
  | 'char_box'
  /**
   * **방에 붙박이**(똥·하루 소품·매트·가방). 발끝선 높이 · 무대 한가운데에서 옆으로 `offset`.
   *
   * ★ 아이를 안 본다 — 걸어도 자세를 바꿔도 그 자리다(상훈님 2026-09-13
   *   "캐릭터가 움직인다고 똥도 같이 움직이면 안돼"). 옛 `foot_front` 는 기준이 **그 아이의 발**이라
   *   아이를 따라다녔다. 규격 문서가 이 이름으로 바뀌는 중이다.
   */
  | 'room_fixed'
