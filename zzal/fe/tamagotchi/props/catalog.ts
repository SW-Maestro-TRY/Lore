...
export const PROP_SPECS: Record<string, PropSpec> = {
  ...
  trash: {
    ...
    stages: [
      // ★★ **덩이 한 개의 크기는 네 단계가 모두 같다 — 주먹밥(`bowl`)과 같은 방식이다.**
      //   (상훈님 2026-09-21: *"1단계 똥크기랑 2단계에서 똥 1개의 크기가 같았으면. 마치 주먹밥처럼."*)
      //   단계 그림마다 **덩이가 그려진 크기가 달라서** ratio 도 달라진다. 맞추는 법:
      //     ratio(n) = 덩이 한 개의 목표 크기(K) x 그림 폭(srcW) / 그 그림 안 덩이 한 개의 폭(원본 px)
      //   원본 덩이 폭은 **화소로 실측**했다(갈색 화소의 가로 뭉치 — 김·파리·가스는 뺀다):
      //     1단계 165 / 2단계 185(2개) / 3단계 158.3(3개 평균) / 4단계 119(4개)
      //   목표 = **K x 0.279** — 390 화면에서 덩이 폭 109px, 아이 **발 폭(양발 바깥 사이)의 1.42배**다.
      //   ⚠️ 이 값이 **지금 그림으로 갈 수 있는 최대**다. 더 키우면 3단계 그림이 무대 폭을 넘는다
      //     (0.279 에서 3단계가 384.1px, 390 화면의 무대가 388px).
      //   ⚠️⚠️ **4단계만 못 맞췄다.** 4단계 그림은 덩이 4개를 제 폭의 5.8배에 퍼뜨려 그려서,
      //     덩이를 0.279 로 맞추면 그림이 **634.8px** 가 되어 무대(388px)를 통째로 벗어난다
      //     (실측: 양쪽 끝 덩이가 화면 밖으로 나가고 가스와 파리만 남는다). 그래서 4단계는
      //     오늘 값 그대로 두었고 덩이가 **28% 작다**. 맞추려면 **4단계 그림을 다시 뽑아야 한다**
      //     (덩이를 크게·간격을 좁게). 상훈님 판정 대기.
      //
      ...
      // ★ 1·2단계 내림 0.08 — 아랫변이 발끝선에 딱 붙어 있어 아이가 그 위에 선 것처럼 보였다.
      //   0.08K = 390 화면에서 31.3px(말풍선이 떠 아이가 낮아진 상태에서는 22.7px — 판정 4 추천값).
      //   ⚠️ 1단계만 내리면 1→2 단계에서 위로 튄다. 둘을 같이 내린다.
      { n: 1, key: "trash_1", ver: 1, file: "trash_1.v1.webp", ratio: 0.301, srcW: 178, srcH: 247, sinkK: 0.08 },
      { n: 2, key: "trash_2", ver: 1, file: "trash_2.v1.webp", ratio: 0.656, srcW: 435, srcH: 315, sinkK: 0.08 },
      { n: 3, key: "trash_3", ver: 1, file: "trash_3.v1.webp", ratio: 0.9817, srcW: 557, srcH: 362, sinkK: 0.565 },
      // ⚠️ 4단계만 옛 비율(덩이가 28% 작다) — 위 머리말 참고. 그림을 다시 뽑으면 1.6224 로 바꾼다.
