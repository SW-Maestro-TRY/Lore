 ...
 *   이라 신호가 그대로 남는다.
 */
// ★ **비어 있는 것이 지금의 정답이다**(2026-09-20). 목욕의 "위에서 물 떨어지는" 단계(`shower`)를
//   상훈님 결정으로 **삭제**했다 — 목욕은 1·2층 모두 거품 3단계 12프레임으로 끝난다.
//   먼지도 `dust_3` 그림이 생기면서 걷힘 바퀴가 없어졌다. 그래서 지금 걷힘을 쓰는 줄은 하나도 없다.
//   ⚠️ 장치는 남겨 둔다 — 표가 "그림이 없는 마지막 단계"를 다시 쓰게 되면 이 한 줄로 되살아난다.
//   쓰는 줄이 없으므로 `plan.sweep` 은 늘 false 이고, 걷힘 바퀴도 안 붙는다.
export const SWEEP_SIGNAL: Record<string, string> = {};
...
// ── 행동 한 판의 바퀴 수 ────────────────────────────────────────────────
//
// ★ **규칙은 하나다 — 한 단계당 4프레임(= 한 바퀴 = 1.8초).** 단계가 없으면 1단계로 본다.
//   (상훈님 2026-09-20 확정. 9/16 의 "비단계 행동은 4바퀴로" 를 **되돌린 것**이고, 되돌리고 나니
//    9/13 계약본 `소품-상황표-v1.json` 이 원래 적어 둔 "단계가 1개면 4프레임" 과 다시 맞는다.)
// ★ **범위는 돌보기만이 아니라 전부다** — 대화 답하기·깨우기·게임 결과·해금 폭죽·나음·방 입장·
//   공유까지 `DEFAULT_ACTION_CYCLES` 에 매달린 것 전부가 4프레임이다.
//   ⚠️ 예외는 **좌우 맞히기의 판 사이 박자**뿐이다 — 그건 게임 재설계(2026-09-20 안 1)가 정하고
//     `useYeoul.ts` 의 `GUESS_*_MS` 에 따로 적어 두었다. 이 상수를 건드려도 게임은 안 흔들린다.
// ★ **단계 소품이 있는 줄은 표(`stages`)가 이긴다** — 밥 3단계·먼지 3단계·거품 3단계가 거기서
//   저절로 12프레임이 된다. 아래 표는 **단계 그림이 없는 층**을 위한 것이다(밥 2층은 소품이
//   없는데도 길이가 1층과 같아야 한다 — "같은 행동은 층이 달라도 길이를 같게").
// ★ 자산은 하나도 안 바뀐다. 캐릭터 움짤은 **전부 2프레임**이고, "16프레임" 은 그 2프레임을
//   여덟 번 돌린 것이었다. 16 → 4 는 **여덟 번을 두 번으로** 줄이는 것이라 끊길 그림이 없다.
...
export const ACTION_CYCLES: Partial<Record<ActionKey, number>> = {
   ...
   *  (1층이 주먹밥 3단계 3->2->1 이라 12프레임). */
  feed_rice: 3,
  /** 청소하기 — 단계형이라 실전에서는 `planCycles`(먼지 3단계 = 3바퀴)가 이긴다.
   *  이 3은 규격이 줄어 단계가 사라졌을 때만 쓰는 안전값이고, **1·2층 모두 12프레임**이라는
   *  결정(2026-09-20)과 같은 값으로 맞춰 둔다. */
  clean: 3,
  /** 목욕하기 — 위와 같은 안전값. 거품 3단계 = 12프레임. */
  bath: 3,
};

/** 단계가 없는 행동의 기본 = **한 바퀴(4프레임 1.8초)**. 위 머리말의 규칙 그대로다. */
export const DEFAULT_ACTION_CYCLES = 1;
