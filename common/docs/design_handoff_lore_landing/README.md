...
## Overview
Lore는 "우리만의 캐릭터로 노는 만화 플랫폼"입니다. 사진 한 장에서 캐릭터 시트를 만들고, 그 시트로 **4컷 만화(Zzal) → 예고편 만화(Trailer) → 스크롤 웹툰(Story)** 순서로 이어가는 서비스입니다.
...
- 좌: 워드마크 `LORE.` — Archivo 900 / 22px / letter-spacing -0.03em. 마침표만 `--accent-zzal`
- 우: 네비 `Zzal · Trailer · Story` — 14px / 500 / `--text-muted`, hover 시 `--text` + 배경 `--surface-2`, radius 999px, padding 8px 10px
...
- 좌측:
  - 배지 pill: 배경 `--bg`, 1px `--border`, radius 999px, padding 7px 14px, 12.5px/500 `--text-muted`, 앞에 7px 원형 dot `--accent-zzal` — 문구: **우리만의 캐릭터로 노는 만화 플랫폼**
...
- 우측: 이미지 콜라주 2열 그리드 gap `clamp(10px,1.6vw,16px)`, radius 22px
  ...
  - 우 상: 1/1 `--tint-zzal` — 4-CUT / 우 하: 4/3 `--tint-trailer` — TEASER
...
#### 3. 섹션 인트로
- 라벨: IBM Plex Mono 11.5px / letter-spacing 0.18em / uppercase / `--accent-story` — `Zzal → Trailer → Story`
...
|---|---|---|---|---|---|---|
| 01 | Zzal (시작점) | `--tint-zzal` / `--accent-zzal` 22% | `01 Zzal · 시작점` | 한 방에 웃기는 / 4컷 만화 | 오늘 있었던 일을 우리 애한테 시켜보세요. 대사 두 줄이면 기승전결 4컷, 그대로 타임라인에 올릴 수 있는 크기로. | 2×2 정사각 그리드(radius 14), 4번째 칸은 `--accent-zzal` 채움 + `4 / PUNCH` |
...
> 순서는 반드시 **Zzal → Trailer → Story**. 사용자 여정(가볍게 시작 → 선택적 티저 → 본편)을 표현한 것이라 알파벳/기존 순서로 바꾸면 안 됩니다.

#### 5. "캐릭터 시트 하나가 세 탭을 굴립니다"
- 섹션 배경 `--surface-2`, 상하 1px 보더. 2열 `repeat(auto-fit, minmax(300px,1fr))`, gap `clamp(26px,5vw,56px)`
- 좌: 라벨 `One character sheet`(`--accent-zzal`) + H2 900 `clamp(26px,6vw,42px)` + 본문 max-width 34ch
- 우: 3개 스텝 카드 세로 스택 gap 12px — 배경 `--bg`, 1px `--border`, radius 20px, padding 18px 20px
  - 번호 원형 배지 30×30, radius 999px, 흰 글자, Archivo 800 13px — 01 `--accent-story` / 02 `--accent-zzal` / 03 `--accent-trailer`
...
- 버튼: 「사진 올리기」 배경 `--accent-zzal` / 「둘러보기」 흰색 35% 아웃라인

#### 8. 푸터
워드마크 + Zzal/Trailer/Story 링크 + 우측 `© 2026 LORE — 우리만의 캐릭터로 만드는 만화`. 12.5px, `--text-subtle`, 상단 1px 보더.
...
| `--accent-zzal` | `#FF6B45` | `#FF7E5C` | Zzal 탭, 포인트 dot, CTA 버튼 |
| `--accent-trailer` | `#1C7866` | `#3FA792` | Trailer 탭 |
| `--tint-story` | `#EDE6FF` | `rgba(123,84,255,0.14)` | Story 계열 연한 면 |
| `--tint-zzal` | `#FFF2E4` | `rgba(255,126,92,0.12)` | Zzal 카드 배경 |
...
- 네비/카드/버튼 앵커는 해당 섹션으로 스무스 스크롤. 실제 서비스에서는 `/zzal`, `/trailer`, `/story` 라우트로 교체.
