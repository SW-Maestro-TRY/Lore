...
DB 마이그레이션 파일(`apps/api/src/main/resources/db/migration/`)은 **날짜 번호**로 붙인다.

```
V<YYYYMMDD>_<HHMM>__<설명>.sql

예시) V20260921_1430__zzal_motion_wish.sql
```

| 항목 | 규칙 |
| --- | --- |
| 형식 | `V` + 날짜 8자리 + 밑줄 하나 + 시각 4자리 + 밑줄 둘 + 설명 + `.sql` |
| 설명 앞 | 서비스 접두 유지 (`zzal_` · `webtoon_` · `trailer_`) |
| 기존 파일 | `V1` ~ `V23` 은 그대로 둠, 재번호 금지 |
| 충돌 판정 | 번호 순서가 아니라 **같은 번호 금지**가 기준 |
| 같은 분에 두 개 | 뒤에 만든 쪽이 시각을 1분 올려서 회피 |

**왜 순번이 아닌가** — 기능 브랜치가 `develop` 과 `staging` 두 줄기로 들어가므로 양쪽에서 순번이 따로 붙는다.
순번 방식이면 운영에 높은 번호가 먼저 적용된 뒤 낮은 번호가 도착해 서버 기동이 멈춘다.
`spring.flyway.out-of-order: true` 로 늦게 온 낮은 번호도 적용되게 열어 뒀고, 날짜·시각을 쓰면 번호가 겹칠 일이 없다.
