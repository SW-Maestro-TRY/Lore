...
# 장면 점검 8항목 중, 재시도로도 안 풀리면 **사람이 봐야 하는** 것.
# 나머지(대사 없음 등)는 메모로만 남기고 넘어간다 — 고칠 수 있는 흠이지
# 작가가 준 것이 사라졌다는 신호는 아니기 때문이다.
#
# '설정 증발'이 여기 있는 이유: 작가가 준 한 줄의 낱말이 결과에서 사라졌다는
# 뜻이다. 실제로 "여성"이 사라진 채로 status=ok 가 되어 웹툰과 이미지까지
# 그대로 내려갔다. 검출해 놓고 통과시키는 것이 검출 안 하는 것보다 나쁘다.
SCENE_BLOCKING_CHECKS = {"설정 증발", "출처 단일"}
...
def check_scenes(scenes: list, idea: str, setting_text: str,
    ...
    # 2. 내가 준 설정이 장면에 살아 있다
    idea_tokens = [t for t in tokens_of(idea) if len(t) >= 2]
    if idea_tokens:
        survived = [t for t in idea_tokens if token_survives(t, joined)]
        if len(survived) * 2 < len(idea_tokens):
            lost = [t for t in idea_tokens if t not in survived]
            hit(2, "설정 증발",
                f"작가가 준 한 줄의 낱말 {len(idea_tokens)}개 중 {len(survived)}개만 남았습니다. "
                f"사라진 것: {lost[:8]}",
                f"[작가가 처음 준 한 줄]의 구체적 낱말({', '.join(lost[:5])})을 "
                "추상어로 뭉개지 말고 장면 안에서 눈에 보이게 움직이게 하세요.")
