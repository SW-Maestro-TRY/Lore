...
**생성 파이프라인은 2026-09-12에 `webtoon/ai/` 로 옮겼습니다.**
거기서 작업할 때는 `webtoon/CLAUDE.md` 가 자동으로 읽힙니다 — 작업 규칙
(커밋·PR·워크트리·권한·과금 승인)은 이 문서와 같고, 구조와 경로만 그쪽
기준입니다. 예전에 이
...
- 2026-09-01 — **그림 한 장이 되는 단위를 장면에서 사건으로** 내렸다 (이슈
  ...
  있어서 한 장이 산만해지던 것을, 구체화 단계가 `scenes[].events[]` 로
  나눠 내게 했다. 이어짐(`continuity`)·`learns`·`guesses`·`leads_to` 도
  전부 사건 단위로 내려가고, 장면에는 `id`·`source`·`function` 만 남는다.
  컷은 여전히 지정하지 않는다 — 사건 하나를 주면 그림이 컷 수·구도·여백·
  대사를 스스로 정한다. **사건 칸이 없는 옛 `detail.json` 은 장면 하나가
...
- 2026-08-29 — **1화를 재미있게 만드는 실험** (이슈 #28/#29): 1화가 매번 같은
  장면으로 나오던 원인이 `p1.json` 의 `trigger_situations` 였음을 실측으로
  확인하고, **씬을 「1화 도입부 집필기」에서 「핵심사건 후보 생성기」로** 바꿨다
  (씬이 후보 4개 → 사람이 선택 → W5 가 집필). 후보에 모순 검사와 블라인드 독자
  검사를 붙였다. 실험은 `arc-first/` 안에서만 돌고, `story-harness` 에는 확인된
  것만 반영했다 (`w4.txt` 의 not_yet 정의, `w5.txt` 의 price_paid·질문 규칙,
  게이트 3종).
  **이어서 작업하기 전에 반드시 읽을 것:**
  [`arc-first/FINDINGS.md`](arc-first/FINDINGS.md) — 무엇을 어떻게 테스트했고
  무엇이 확정됐는지, 다음에 할 일과 반복하지 말아야 할 함정이 정리돼 있다.
