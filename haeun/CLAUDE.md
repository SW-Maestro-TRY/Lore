...
## 이슈 번호 매핑 (Lore 공유 저장소 작업 시)

`haeun/` 바깥, 즉 Lore 공유 저장소(`comic/`, `common/`, `infra/`, `story/`,
`trailer/`, `apps/`)를 고칠 때는 `common/docs/git-convention.md`의 컨벤션을
그대로 따릅니다 — 항상 관련 GitHub 이슈 번호를 정하고, 커밋 메시지를
`[#이슈번호] Type: 내용` 형식으로 남깁니다 (예: `[#14] Feat: 로그인 기능 추가`).
브랜치명은 `[이슈타입]/#이슈번호` (예: `feat/#14`). 이슈가 없는 잡무만 예외
(`Chore: 내용`).

`haeun/`는 아직 git에 올라가 있지 않은(untracked) 개인 작업 폴더라 지금은 이
컨벤션이 적용될 일이 없습니다 — 나중에 haeun/를 저장소에 포함시키기로 하면
그때 다시 정합니다.
