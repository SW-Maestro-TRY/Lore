...
[cap, num, fit, edit, gap, dgap].filter(Boolean).forEach(el =>
...
def _cut_items(
    ...
    for cut in cuts:
        ...
        items.append(
            f'<section class="cut" id="cut{n}" data-cut="{n}" data-size="{_esc(size)}">'
            f'<div class="{frame_cls}"{style}>{inner}</div>'
            f'<div class="no">컷 {n} <i>{_esc(size)}</i></div>'
...
{dtoggle}  <label><input type="checkbox" id="edit" checked> 크기 편집</label>
