...
def test_page_review() -> None:
    ...
    # 2026-09-06 — "연출은 통째로 안 본다" 에서 **취향만** 안 보는 것으로 좁혔다.
    # 같은 컷을 복붙한 것은 취향이 아니라 실패라서 이제 지적한다.
    ok("연출 취향은 안 본다고 말한다", "연출 취향." in text)
    ok("그림이 제 일을 했는지는 본다", "그림이 제 일을 했는가" in text)
...
def test_story_review() -> None:
    ...
    # 2026-09-06 — 기준이 "관계·상황·목표 중 하나가 달라지면 된다" 에서
    # "마지막에서 끊고, 그 질문은 이 화 안에서 답할 수 없어야 한다" 로 올라갔다.
    ok("만들 때 준 기준을 그대로 쓴다", "끝나는 자리가 아니라 끊기는 자리다" in text)
    ok("이 화 안에서 답할 수 없어야 한다고 적는다",
       "이 화 안에서 답할 수 없어야 한다" in text)

    # --- 마무리를 코드가 센다 (2026-09-06) --------------------------------
    #
    # 프롬프트에 적어 놓는 것만으로는 **조용한 마무리가 그대로 통과했다.**
    # 실측: 마지막이 "새로운 시작의 단서를 얻는다" 로 끝난 후보가 통과로
    # 나갔고, 검수가 적은 남는 질문은 "앞으로 어떻게 될지" 였다. 달라진 것도
    # 있고 질문도 있으니 옛 규칙은 다 지나간다.
    def ending_made(one: dict) -> list:
        return SC._ending({"ending": one}, 5)[1]

    ok("두루뭉술한 질문뿐이면 잡는다",
       len(ending_made({"changed": "규칙을 어김", "question": "앞으로 어떻게 될지",
                        "from": "4"})) == 1)
    ok("구체적인 것이 같이 있으면 안 잡는다",
       ending_made({"changed": "규칙을 어김", "from": "4",
                    "question": "이올이 규칙을 어기면서 얻은 새로운 시작의 단서가"
                                " 무엇인지, 앞으로 어떻게 될지"}) == [])
    ...
    # 이 칸들을 묻기 전에 만든 판정 파일이 있다 — 없는 칸을 못 지켰다고
    # 셀 수는 없다. 옛 판정이 전부 주의로 뒤집히면 안 된다.
    ok("옛 판정(새 칸 없음)은 그대로 통과한다",
       ending_made({"changed": "달라짐", "question": "노인이 왜 숨겼나",
                    "from": "3"}) == [])
