...
**그림으로 만들 수 있는가** — 화면에 보이는 것이 하나도 없는 줄("관계가
서서히 달라진다")은 그릴 수가 없다(`kind` 가 `한장`).

한때 여기서 **사건 개수**도 셌다. 한 줄에 사건이 여럿이면 한 장에 안
들어간다고 봤는데, **실측으로 틀린 것이 확인돼서 뺐다.** 한 장은 컷
하나가 아니라 세로로 긴 한 장을 컷 여럿이 나눠 쓰는 자리라
(`prompt/image_prompt.openai` 의 "아래 컷들이 이 한 장을 위에서 아래로
나눠 쓴다"), 사건 넷짜리 장면이 컷 다섯 장으로 하나도 안 버리고 그려졌다
(run 20260904T205354 의 page02·page04). 그 규칙을 켜 두면 정상으로 그려질
후보에 매번 지적이 붙어서, 판정이 신호가 아니라 잡음이 된다.
...
**순위가 아니고, 아무것도 안 막는다.** 재미 자체는 취향이라 셀 수 없지만
**이야기가 움직였는지**는 셀 수 있다. 첫 장면의 문제와 마지막 장면의 문제를
직접 적게 하고 그 둘이 다른지를 보는 것이 `shift` 이고, 나머지 넷이 그 변화가
어디로 갔는지를 나눠 본다.

앞선 다섯(원한다·고른다·몰린다·치른다)을 버린 이유는 실측이다 — 넷이 전부
3점을 받았는데 이야기는 컵라면 흔적 숨기기였다. 조건을 묻는 항목은 작은
사건으로도 만족된다.

지금 단계에서 반려를 안 붙인 이유는, 이 점수가 사람 눈과 맞는지 아직 모르기
때문이다. 반려를 켜면 후보를 다시 뽑는 값과 기다리는 시간이 같이 붙는데,
틀린 잣대로 막으면 좋은 후보가 버려진다. 먼저 몇 판 뽑아 "내가 재미있다고
본 것에 높은 점수가 붙었는가" 를 확인하고, 맞으면 그때 켠다.
...
# 이야기를 굴리는 다섯 — story_prompt 의 「이야기는 문제가 달라지는 것이다」와
# 일대일이다. 2026-09-12 에 한 번 갈아엎었다: 처음 다섯(원한다·고른다·몰린다·
# 치른다·안뻔하다)은 좋은 이야기의 **조건**을 물었는데, 조건은 작은 사건으로도
# 만족돼서 후보 넷이 전부 3점을 받고도 재미가 없었다(컵라면·500원·폐기 도시락).
# 지금 다섯은 **이야기가 움직였는지**를 묻는다 — 첫 문제와 마지막 문제를 직접
# 적게 하고 그 둘이 다른지를 본다.
...
def _ending(one: dict, last_scene: int) -> tuple[dict, list[dict]]:
    ...
    ending = {"changed": _text(raw.get("changed")),
              ...
              # 2026-09-06 추가. 없으면 빈 값이라 옛 판정은 그대로 통과한다.
              "answered_here": _text(raw.get("answered_here")),
              "after_turn": _text(raw.get("after_turn"))}
    ...
    # ---- 여기부터 2026-09-06 추가 --------------------------------------
    #
    # 위 셋만으로는 **조용한 마무리가 그대로 통과한다.** 실측: 마지막 장면이
    # "새로운 시작의 단서를 얻는다" 로 끝난 후보가 통과로 나갔고, 검수가 적은
    # 남는 질문은 "앞으로 어떻게 될지" 였다. 달라진 것도 있고 질문도 있으니
    # 위 규칙은 다 지나간다 — 그런데 읽는 사람에게는 아무것도 안 걸린다.
    #
    ...
    if after and after.lower() not in EMPTY:
        add("major", "뜻이 바뀌는 사건 뒤에 정리하는 사건이 더 붙어 있다 — "
                     f"{after}")

    # "앞으로 어떻게 될까" 류는 질문이 아니라 감상이다. 무엇을 묻는지가 없으면
    # 다음 화를 볼 이유가 안 된다. 낱말로 세는 것이라 **확실한 것만** 잡는다.
    if ending["question"] and _vague(ending["question"]):
...
# 애매한 것을 넓게 잡으면 멀쩡한 질문까지 걸린다.
VAGUE = ("앞으로 어떻게", "어떻게 될지", "어떻게 될까", "어떻게 되는지",
         "어떻게 흘러갈", "무슨 일이 일어날지", "다음이 궁금")


def _vague(question: str) -> bool:
    """질문이 손에 잡히는가 — 잡히면 거짓.

    두루뭉술한 말이 들어 있어도 **그 앞에 구체적인 것이 같이 적혀 있으면**
    잡지 않는다. "이올이 규칙을 어기고 얻은 것이 무엇인지, 앞으로 어떻게
    될지" 는 앞쪽이 구체적이라 질문이 맞다. 두루뭉술한 말 **뿐**일 때만 센다.
    """
    q = question.strip()
    if not any(v in q for v in VAGUE):
        return False
    # 두루뭉술한 말을 걷어내고도 스무 자 넘게 남으면, 남은 쪽이 진짜 질문이다.
    rest = q
    for v in VAGUE:
        rest = rest.replace(v, "")
    rest = "".join(ch for ch in rest if ch not in " ,·、.?？\n")
    return len(rest) < 20


def _scores(one: dict) -> dict:
    """`scores` 를 읽는다. 0~3 밖은 잘라 넣고, 아예 없으면 빈 칸으로 둔다.

    없는 것을 0 으로 채우지 않는다 — "안 왔다" 와 "없다고 판정됐다" 는 다르고,
    0 으로 메우면 검수를 끈 판과 점수가 낮은 판이 같아 보인다.
    """
    raw = one.get("scores")
    if not isinstance(raw, dict):
        return {}
    ...
    for key in SCORES:
        ...
        if isinstance(item, dict):
            value, where = item.get("score"), _text(item.get("where"))
        else:
            value, where = item, ""
        if value is None or (isinstance(value, str) and not value.strip()):
            continue
        out[key] = {"score": max(0, min(3, _int(value))), "where": where}
    return out
...
def parse(text: str, expect: list[int] | None = None) -> dict:
    ...
    for one in obj.get("candidates") or []:
        ...
        # 점수는 `verdict` 에 **안 들어간다.** 지금은 기록만 하는 단계라,
        # 여기에 섞으면 landing 의 자동 고르기(`_auto_pick` 이 verdict 로
        # 거른다)가 조용히 반려 기능이 된다.
        got[n] = {"n": n,
...
def score_line(scores: dict) -> str:
    """`원한다3 고른다1 …` — 사람이 한 줄로 훑는 자리."""
    if not scores:
        return ""
    return " ".join(f"{SCORE_LABEL[k]}{scores[k]['score']}"
                    for k in SCORES if k in scores)
...
def summary(review: dict) -> str:
    ...
    for c in review.get("candidates") or []:
        n = c["counts"]
        line = f"{c['n']}번 {c['verdict']}"
        if any(n.values()):
            line += f"(c{n['critical']}·m{n['major']}·n{n['minor']})"
        scored = score_line(c.get("scores") or {})
        if scored:
            line += f" [{scored}]"
        parts.append(line)
