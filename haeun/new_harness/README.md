...
| `PAGE_REVIEW` | 그린 그림 검수 | gemini · openai · anthropic |
...
한 장을 그릴 때마다 **직전 장과 지금 장 두 그림**을 이야기와 함께 모델에게
주고 한 번 더 읽힌다 (`pagecheck.py`).

```
장면 n 그리기 ──> 검수 ──> 이어짐 ──> 다음 장
                   │
                   └─ 제자리·건너뜀·되돌아감 / critical
                        └─> 그 장을 지우고 지적을 붙여 다시 그린다 (기본 1회)
```

**왜 그리기 전이 아니라 그린 뒤인가.** 컷을 미리 정해 주면 그림이 지시를
옮기기만 해서 결과가 평평해진다(컷 대본 단계를 없앤 이유와 같다). 그래서
자유롭게 그리게 두고, 나온 그림이 이야기와 어긋났을 때만 되돌린다.

**무엇을 보는가 — "장면이 맞는가" 가 아니다.** 한 장이 장면 하나를 정확히
담아야 하는 것이 아니다. 한 장면이 두 장에 걸쳐도 되고, 한 장이 앞 장면의
끝을 마저 그리고 다음 장면으로 넘어가도 되고, 뒤에 올 이야기의 복선을 미리
깔아도 된다. 보는 것은 하나다 — **직전 그림에서 이야기가 이어지는가.**

| `flow` | 뜻 | 다시 그리는가 |
| --- | --- | --- |
| `이어짐` | 앞 장 다음으로 읽힌다 | 아니오 |
| `제자리` | 같은 순간을 한 번 더 그렸다 | 예 |
| `건너뜀` | 안 그린 장면을 통째로 지나쳐 한참 뒤를 그렸다 | 예 |
| `되돌아감` | 이미 지나온 이야기로 돌아갔다 | 예 |

여기에 `critical` 지적(인물이 다른 사람이 됨, 설명 안 되는 구멍)이 하나라도
있으면 역시 다시 그린다. **판정은 모델 말을 안 믿고 코드가 다시 센다** —
`run.parse_review` 와 같은 이유다.

**연출은 판정 대상이 아니다.** 컷 수·구도·카메라·여백·대사·말풍선 모양·SD
사용은 검수가 건드리지 않는다. 그것까지 보면 검수가 곧 컷 지정이 되어, 컷
대본을 없앤 이유가 사라진다. 다시 그릴 때 붙는 지적도 이야기만 짚는다.
...
python3 run.py --run-id <id> --detail-pages --no-page-review
...
```
NH_PAGE_REVIEW=0          검수를 끈다 (기본은 켜짐)
NH_PAGE_REVIEW_RETRY=0    검수는 하되 다시 그리지 않는다 (기본 1회)
PAGE_REVIEW_PROVIDER=...  그림을 보는 호출이라 눈이 있는 모델이어야 한다
```

결과는 `pages/pageNN.review.json`(다시 그린 뒤 것은 `.review.2.json`)에
쌓인다. 원문은 파싱 **전에** 먼저 쓴다.

**검수가 실패해도 그림은 안 버린다.** 호출이 죽거나 응답을 못 읽으면 사유만
기록에 남기고 그 장은 그대로 둔다 — 이미 값을 치른 그림이다. 다시 그리기는
장당 **한 번**까지다(`NH_PAGE_REVIEW_RETRY`). 두 번째도 어긋나면 그 다음이
나아진다는 근거가 없어서 그림 값만 계속 나간다.
