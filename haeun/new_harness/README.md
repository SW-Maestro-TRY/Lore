...
flat  = pages.flatten_cuts(scenes)              # cuts.json 을 편다
pgs   = pages.group_pages(flat)                 # 페이지로 묶는다
texts = imageprompt.page_prompts(pgs, sheets=[imageprompt.sheet_line(spec)])
```

콘티는 사람이 읽는 표기(`상반신 / 정면 / 옆모습`)로 나온다. 그대로 보내지
않고 문장으로 편다 — 모델이 슬래시를 셋으로 나눠 읽는다는 보장이 없고,
무엇보다 **대사의 종류가 곧 말풍선 모양**이라 그 번역은 코드가 해야 한다.
...
| 생각 | 구름 모양 + 꼬리 |
| 외침 | 뾰족한 형태 + 꼬리 |
| 화면밖 | 꼬리를 컷 바깥으로 (꼬리 안 붙임) |
| 전화 · 라디오 | 각진 형태 |
| 나레이션 | 네모 상자, 컷 위쪽 모서리 |
| 효과음 | 말풍선 없이 글자만 |
| 글 | 말풍선 아님 — 적힌 화면·종이를 그린다 |

높이 비율은 `tiny 1 · small 2 · normal 3 · large 5 · full 페이지 전체`.

**대사 글자는 한 글자도 안 바꾼다.** 형식이 안 맞는 줄은 모양을 못 붙인 채로
원문 그대로 내보낸다 — 지우는 것보다 낫다.
...
- **LD 화풍이 임시값이다.** `prompt/image_prompt` 의 `- LD:` 줄이 지금
  ...
  `styles` 에 다 쓰인 그림체가 일곱 개 있다 — 옮겨 오면 되는 자리다.)
- **한 페이지 최대 컷 수.** 지금은 개수 5개로만 끊는다. 높이 비율 합계로 끊는
  ...
  그게 곧 기준이 된다.
- **같은 공간이 여러 페이지에 걸칠 때.** 가장 넓게 나오는 컷을 먼저 그려서
  나머지 호출에 참조로 붙이는 방법 — 아직 안 해 봤다.
- **말풍선 위치.** 고정 블록은 `위치 {위치}` 를 받게 돼 있는데 콘티가 그 값을
  안 낸다. 지금은 그 자리를 비우고, 고정 블록의 "위에서 아래 순서로 읽히게
  배치한다" 에 맡긴다.
