...
### 고르기 전 검수 — 이 후보를 그리면 어디서 걸리는가

후보 4개를 다 만든 뒤, 한 번 더 **처음 읽는 독자의 눈으로** 읽힌다
(`storycheck.py`). 어느 후보가 나은지는 안 본다 — 그건 사람이 고르는
자리다. 보는 것은 "이 후보를 그대로 그리면 독자가 어디서 걸리는가" 하나다.

```
후보 4개 ──> 검수 ──> 판정을 카드 옆에 붙여 사람에게 보여준다
                       (아무 후보도 막지 않는다)
```

여기에 이 파이프라인에만 있는 눈금이 둘 더 붙는다.

**그림으로 만들 수 있는가** (`kind` 가 `한장`) — "관계가 서서히 달라진다"
처럼 화면에 보이는 것이 하나도 없는 줄은 그릴 수가 없다.
**사건 개수는 안 센다** — 한 장은 컷 하나가 아니라 컷 여럿이 나눠 쓰는
자리라, 사건 넷짜리 장면이 컷 다섯 장으로 멀쩡히 그려진다(실측:
run 20260904T205354 의 page02·page04).

**마지막에 다음 화가 궁금해지는가** (`kind` 가 `마무리`) — 1화 마지막에
관계·상황·목표 중 하나가 달라져 있고 그 변화 때문에 질문 하나가 남는가.
`story_prompt` 의 「10」이 만들 때 요구한 것을 볼 때도 그대로 쓴다.

이 자리에 두는 이유는 **되돌리는 값이 가장 싸서**다. 후보 뒤에는 컷을
나누고 대사를 쓰는 단계가 없어서 여기 난 구멍은 뒤에서 안 메워지고, 다
그리고 나서 알면 이미 장당 그림값을 다 치른 뒤다.

    NH_STORY_REVIEW=0    끈다 (기본은 켜짐)
    python3 storycheck.py --run-id <id>   # 이미 만든 후보를 검수만

결과는 `story_review.json`.
...
이 화를 다 그리고 나서 페이지 **전부**를 한 번에 주고, "아무것도 모르는
사람이 이 화면들만 넘겨 보고 이해하는가" 를 묻는다 (`episodecheck.py`).

바로 위 검수(`pagecheck`)는 **인접한 두 장**만 본다. 장장이 전부 매끄러워도
다 읽고 나서 무슨 이야기였는지 말할 수 없는 일은 거기서 그대로 통과한다 —
이어짐은 이야기가 있다는 뜻이 아니다.

**다시 그리지 않는다.** 화 전체가 걸렸을 때 고치는 길은 한 장을 다시
그리는 것이 아니라 이야기를 다시 고르는 것이고, 그건 사람이 정할 일이다.
판정은 완성본 화면에 「처음 보는 사람에게는 이렇게 읽혔습니다」로 붙는다.

그래서 이 검수는 **장마다 보는 검수가 일부러 안 보는 것도 본다** — 깨진
글자가 대표다. `pagecheck` 은 그것 때문에 다시 그리면 손해라서 안 보지만,
여기서는 다시 그릴 일이 없으니 알려 주기만 하면 된다.

한 장만 다시 그릴 때(`--page 3`)는 안 부른다 — 화 전체를 보는 검수라
일부만 새로 그린 상태에서는 볼 것이 못 되고, 한 장 고칠 때마다 화 전체
값이 또 나간다.

    NH_EPISODE_REVIEW=0    끈다 (기본은 켜짐)
    python3 episodecheck.py --run-id <id>   # 이미 그린 것을 검수만

결과는 `episode_review.json`.
