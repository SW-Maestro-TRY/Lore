...
## run.py 에 붙어 있다 (기본은 꺼짐)

`stage_story` 가 `storycheck` 다음에 이어서 부른다 — 후보 검수와 같은
자리, 같은 순서(만들고 나서 본다). **기본은 꺼짐**이다. 이 자가 사람 눈과
맞는지 아직 확인되지 않았고, 맞는지 보기 전에 제품 run 마다 글 호출을
하나 더 붙일 이유가 없기 때문이다 — `axes_enabled`/`engines_enabled` 를
번갈아 켜고 끄며 비교했던 것과 같은 순서를 밟는다: 먼저 켜서 눈금을 맞춘
뒤에 기본을 정한다.

`.env` 에 `NH_STORY_DIFF=1` 로 켠다.
...
def enabled() -> bool:
    """기본은 **꺼짐.** 켜려면 `.env` 에 `NH_STORY_DIFF=1`.

    `storycheck`(기본 켜짐)과 반대다 — 이 자가 사람 눈과 맞는지 아직 확인된
    적이 없어서, 맞는지 보기 전에 제품 run 마다 글 호출을 하나 더 붙일
    이유가 없다. `axes_enabled`/`engines_enabled` 를 번갈아 켜고 끄며 먼저
    비교하고 나서야 기본값을 정했던 것과 같은 순서다(`run.py`). 몇 판
    돌려서 "사람이 겹친다고 느낀 짝에 실제로 겹침이 붙었는가" 를 확인한
    뒤에 기본을 켜짐으로 옮긴다.
    """
    return sc._text(llm.env("NH_STORY_DIFF") or "").lower() in ("1", "on", "true", "yes")
