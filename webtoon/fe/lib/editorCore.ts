// @ts-nocheck
/* 편집실 엔진 — haeun/landing/web/editor.js 를 그대로 옮겼다.
 *
 * **베낀 것이 아니라 옮긴 것이다.** 원본은 여러 번 손본 끝에 확정된 코드라
 * (끌기·크기 조절·회전·지난 판 되돌리기·다시 그리기·굽기), 타입을 붙이자고
 * 다시 쓰면 그 과정에서 미묘하게 달라진다. 그래서 본문은 손대지 않고 다음
 * 네 곳만 바꿨다:
 *
 *   1. API 주소 — `/api/...` → `${API}/...` (앞에 스프링이 선다, webtoon/be)
 *   2. 어느 작품인가 — 주소(?run=&ep=) 가 아니라 **부모가 넘겨준 값**
 *   3. 화면 이동 — `location.search = ...` 가 아니라 부모가 준 길(openRun)
 *   4. 걷어내기 — document 에 매단 것을 화면을 떠날 때 뗀다
 *
 * 그래서 `@ts-nocheck` 이다. 이 파일은 확정된 브라우저 코드의 이식본이고,
 * 여기에 타입을 얹는 것은 이식이 아니라 재작성이다.
 *
 * DOM 을 직접 만지는 것도 원본 그대로다 — 끌기·크기 조절·회전·그 자리에서
 * 글 고치기는 포인터 이벤트를 원본과 똑같이 다뤄야 정확히 같게 움직인다.
 */
...
/**
 * 편집실을 이 화면에 올린다. 돌려주는 함수를 부르면 매단 것을 전부 걷는다.
 *
 * @param openWith 어느 작품의 어느 회차를 열 것인가
 * @param openRun  다른 작품·회차로 건너가는 길 (원본의 주소 바꾸기 자리)
 */
export function mountEditor(
  openWith: EditorOpen = {},
  openRun: (runId: string, episode: number) => void = () => {},
): () => void {
  /* 걷어낼 것들. 원본은 페이지가 통째로 사라져서 걷을 일이 없었다. */
  const bound: [EventTarget, string, EventListener][] = [];
  ...
  /* 서버가 주는 주소는 자기 기준(`/api/...`)이다. 여기서는 그 앞에 스프링이
     서 있으므로 접두사를 갈아 끼운다 — 안 바꾸면 그림도 내려받기도 404 다.
     (우리가 직접 만든 주소는 이미 API 로 시작하므로 그대로 지나간다.) */
  function atApi(u) {
    return typeof u === "string" && u.startsWith("/api/") ? API + u.slice("/api".length) : u;
  }

  function rawImg(s) {
    const u = atApi(s.image || "");
    if (!RUN_ID || !u.includes("/runs/")) return u;
  ...
  function itemHTML(it) {
    ...
    /* 말풍선은 **SVG 한 장**으로 그린다.
       CSS 로는 몸통(둥근 네모)과 꼬리(테두리 세모)를 따로 그릴 수밖에 없어서,
       둘이 만나는 자리에 늘 틈이 보였다 — 타원은 가장자리가 안으로 휘므로
       세모가 공중에 뜬 것처럼 보인다. 한 덩어리로 그리면 그 틈이 없고,
       무엇보다 꼬리를 **아무 방향으로나** 뻗을 수 있다. */
    const inner =
      it.type === "bubble"
  ...
  function setDock(open) {
    ...
    // 원본은 이 표시를 body 에 붙였다(혼자 뜨는 페이지였다). 여기서는 앱
    // 안이라 Lore 앱의 body 를 건드리지 않고 편집실 자기 요소에 붙인다.
  ...
  async function paintWorks(current) {
    ...
    host.addEventListener("click", e => {
      ...
      openRun(card.dataset.run || "", Number(card.dataset.ep) || 1);
    });
  }
  ...
  /* 원본은 DOMContentLoaded 를 기다렸다. 여기서는 React 가 화면을 붙인 뒤에
     부르므로 곧장 시작하되, 안이 async 라 한 겹 감싼다. */
  void (async () => {
    // 어느 작품의 어느 회차인가는 **부모가 준다**. 원본은 독립 페이지라
    // 주소(?run=&ep=)에서 읽었지만, 여기서는 앱 안의 한 화면이다.
