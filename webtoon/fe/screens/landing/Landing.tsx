...
const FAQ: { q: string; a: React.ReactNode }[] = [
  { q: "이건 뭐하는 서비스인가요?", a: <>사진 한 장과 이름만 주시면 <b>웹툰 한 화가 통째로</b> 나오는 스튜디오예요. 캐릭터를 그리고, 이야기를 짓고, 표지부터 마지막 장까지 순서대로 그려 드립니다. 그림을 못 그려도, 이야기를 안 써 봤어도 괜찮아요.</> },
  { q: "AI SW 마에스트로란 무엇인가요?", a: "과학기술정보통신부가 주관하고 정보통신기획평가원(IITP)이 운영하는 AI 소프트웨어 인재 양성 과정이에요. LORE는 이 과정에서 실제로 기획하고 만들고 있는 프로젝트입니다." },
  { q: "마음에 안 들면 다시 만들 수 있나요?", a: <>네, 가능해요. <b>「2번 확인하며」</b>를 고르면 캐릭터 시트와 이야기 단계에서 확인하면서 다시 만들 수 있고, 완성한 뒤에도 편집실에서 컷 단위로 다시 그릴 수 있어요.</> },
  ...
  { q: "완성한 웹툰은 다른 사람도 볼 수 있나요?", a: <>기본적으로 <b>둘러보기</b>에 공개돼요. 마이페이지에서 언제든 비공개로 바꿀 수 있어요.</> },
...
export default function Landing({ go }: { go: Go }) {
  ...
  const jobLabel = job
    ? (job.art?.total ? `${job.art.done} / ${job.art.total}장` : job.stage_label)
    : "";
  ...
  const allowanceText = api.allowanceLine(allowance);
  ...
  useEffect(() => {
    ...
    api.browseRuns()
      ...
      .catch((e: Error) => { if (alive) setRunsErr(e.message || "작품을 못 불러왔습니다"); });
    return () => { alive = false; };
  }, [tries]);
  ...
  return (
    <div className="wt-landing">
      ...
      <section className="wt-landing-hero">
        {job && (
          <button type="button" className="btn btn-w wt-landing-jobpill" onClick={() => go("running", { job: job.id })}>
            ...
            만들던 웹툰 <span className="dim" style={{ fontWeight: 400 }}>· {jobLabel}</span>
          </button>
        )}
        <h1>AI 웹툰 제작 서비스 LORE</h1>
        <p className="muted">{phone ? "캐릭터 · 사진 · 그림 · 좋아하는 사람" : "캐릭터 · 사진 · 그림 · 그 무엇이든!"}</p>
        <button type="button" className="btn btn-p wt-landing-cta" onClick={start}>지금 시작하기</button>
        {allowanceText && <span className="wt-landing-allow">{allowanceText}</span>}
      </section>

      {/* 예시 작품 띠 */}
      <section className="wt-landing-works">
        <span className="muted wt-landing-works-label">이 서비스로 만들어진 편</span>
        {runsErr ? (
          <div className="wt-landing-works-err">
            <span className="err">{runsErr}</span>
            <button type="button" className="btn btn-w btn-sm" onClick={() => setTries((n) => n + 1)}>다시 시도</button>
          </div>
        ) : !runs ? (
              ...
              {marqueeList.map((r, i) => (
                <figure key={`${r.run_id}-${i}`} className="wt-landing-fig">
                  <a href={hrefOf("result", { run: r.run_id })} onClick={(ev) => { ev.preventDefault(); go("result", { run: r.run_id }); }}
                    ...
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img className="cover wt-landing-cover" src={api.coverUrl(r.run_id, r.cover_page ?? 1, r.cover_episode ?? 1, !!r.example)} alt={`${titleOf(r)} 표지`} />
      ...
      <section className="wt-landing-steps">
        <h2>{phone ? "고르면, 이렇게 만들어져요" : "나만의 이야기를 만들어 보세요!"}</h2>
            ...
            <div className="wt-landing-step-box">
              <div className="wt-landing-step-form">
                <div className="wt-landing-step-name">
                  ...
                  <div className="wt-landing-step-input">세라핀</div>
                </div>
                <div className="wt-landing-step-chips">
                  {["로맨스", "판타지", "액션", "일상", "스릴러"].map((g, i) => (
                    <span key={g} className={i === 0 ? "on" : ""}>{g}</span>
                  ))}
                </div>
                <div className="wt-landing-step-btn">웹툰 만들기</div>
              </div>
      ...
      <section className="wt-landing-needs">
        <h2>{phone ? <>이야기가 웹툰이 되는 과정,<br />LORE 하나로 충분합니다</> : <>이미지 한장이 웹툰이 되는 과정,<br />LORE 하나로 충분합니다.</>}</h2>
        <div className="wt-landing-needs-row">
          <a href={hrefOf("create")} className={`wt-landing-need${need === 0 ? " on" : ""}`}
            ...
            <div className="wt-landing-need-text">
              <b>내 캐릭터가 살아 움직이는 걸 보세요</b>
              <span className="muted">
                {phone
                  ? "설정만 있던 캐릭터가 이야기 속에서 말하고 움직여요. 캐릭터를 넣으면 그 캐릭터가 주인공인 웹툰이 나옵니다."
                  : <>설정만 있던 캐릭터가 이야기 속에서 말하고 움직여요.<br />캐릭터를 넣으면, 그 캐릭터가 주인공인 웹툰이 나옵니다.</>}
              </span>
              <div className="wt-landing-need-more"><p className="muted">{NEED_LONG[0]}</p></div>
            </div>
            <div className="wt-landing-need-fig">
              <EditorMock feat={0} s={phone ? 0.7 : 0.9} height={phone ? 260 : 400} who="세이엘" />
            </div>
          </a>
          ...
          <a href={hrefOf("try")} className={`wt-landing-need${need === 1 ? " on" : ""}`}
            ...
            <div className="wt-landing-need-text">
              <b>{phone ? "뭐든 넣으면 웹툰 속 캐릭터가 돼요" : "어떤 캐릭터가 나올지, 뽑아볼까요?"}</b>
              <span className="muted">
                {phone
                  ? "내 사진도, 최애도, 강아지도, 아무것도 없어도 돼요. 의인화 없이 그대로 웹툰 세계관에 들어가요."
                  : <>사진을 넣어도, 이야기를 적어도, 아무것도 없이 시작해도 좋아요.<br />당신이 고른 세계관에 맞춰 새로운 캐릭터를 만들어드려요.</>}
              </span>
              <div className="wt-landing-need-more"><p className="muted">{NEED_LONG[1]}</p></div>
            </div>
            <div className="wt-landing-need-fig">
              <div className="wt-landing-cut">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={CUT_IMG} alt="웹툰 한 컷" />
                <span className="wt-landing-genre" style={{ left: 12, top: 12, fontSize: 11, padding: "3px 9px" }}>로판</span>
                <div className="wt-landing-bubble" style={{ right: 14, top: 18, maxWidth: "58%", borderRadius: 14, padding: "7px 11px", fontSize: phone ? 11 : 12.5 }}>멍!</div>
                <div className="wt-landing-cap" style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
                  <b style={{ fontSize: phone ? 13 : 14.5, lineHeight: 1.4 }}>몽이는 이 로맨스 웹툰에서, 강아지인 채로 악역 영애예요</b>
                  <span style={{ fontSize: phone ? 11.5 : 12.5, opacity: 0.85 }}>이 캐릭터로 1화 보기</span>
                </div>
              </div>
            </div>
          </a>
      ...
      <section className="wt-landing-promise">
        <h2><span className="wt-landing-hl">웹툰 1화를 위해서</span><br />LORE가 약속 하는 세 가지</h2>
        <div className="wt-landing-promise-grid">
          <EditorMock feat={feat} s={phone ? 0.8 : 1} height={phone ? 360 : 560} who="몽이" />
          <div className="wt-landing-promise-list">
            {PROMISES.map((p, i) => (
              <div key={p.n} className={`wt-landing-promise-item${feat === i ? " on" : ""}`}
                ...
                <div>
                  <b>{p.t}</b>
                  <div className="wt-landing-promise-more"><p className="muted">{p.d}</p></div>
                </div>
              </div>
            ))}
      ...
      <section className="wt-landing-last">
        <h2>{phone ? <>당신의 이야기를<br />기다리고 있어요</> : "당신의 이야기를 기다리고 있어요"}</h2>
        <button type="button" className="btn btn-p wt-landing-cta" onClick={start}>지금 시작하기</button>
      </section>
        ...
        <div className="wt-landing-faq-list">
          {FAQ.map((f, i) => (
            <details key={f.q} open={i === 0}>
              <summary>{f.q}<span className="wt-landing-faq-ic"><IconPlus size={20} /></span></summary>
      ...
      <footer className="wt-landing-foot">
        ...
        <div className="wt-landing-badges">
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <span><img src="/static/badges/msit-icon.png" alt="" />과학기술정보통신부</span>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <span><img src="/static/badges/iitp-icon.png" alt="" />정보통신기획평가원(IITP)</span>
        </div>
