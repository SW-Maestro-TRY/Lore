...
export default function GuestLimit({ reason, loggedIn, go }: { reason: string; loggedIn: boolean; go: Go }) {
  ...
  return (
    <div className="wt-prog">
      <MobileTop back={{ href: "", onClick: () => go("landing") }} title="만들기" />
        ...
        <div className="wt-prog-limit">
          {!loggedIn ? (
            <div className="card">
              ...
              <img src={art} alt="" />
              <span className="num">게스트</span>
              <h2>오늘 무료 편수를 다 썼어요</h2>
              <span className="muted">
                {reason ? `${reason} ` : ""}게스트는 하루 2편까지 무료예요. 로그인하면 크레딧으로 바로 이어서 만들 수 있어요. 지금까지 만든 것은 그대로 남아요.
              </span>
              <button type="button" className="btn btn-p" onClick={() => go("mypage")}>로그인하고 이어서 만들기</button>
              <button type="button" className="btn btn-w" onClick={() => go("works")}>둘러보기 하며 내일 다시</button>
            </div>
          ) : (
            <div className="card">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={art} alt="" style={{ opacity: 0.7 }} />
              <span className="num" style={{ color: "#a13a2e" }}>전체 마감</span>
              <h2>오늘은 여기까지예요</h2>
              <span className="muted">{reason || "오늘 만들 수 있는 전체 편수가 찼어요. 자정이 지나면 다시 만들 수 있어요."}</span>
