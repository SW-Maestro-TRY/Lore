...
export default function Works({ go, authenticated }: { go: Go; authenticated: boolean }) {
  ...
  const chips = (
    <div className="wt-works-chips">
      <button type="button" className={`chip${filter === "all" ? " on" : ""}`} onClick={() => setFilter("all")}>전체</button>
      <button type="button" className={`chip${filter === "mine" ? " on" : ""}`} onClick={() => setFilter("mine")}>내 작품</button>
      {genres.map((g) => (
        <button key={g} type="button" className={`chip${filter === g ? " on" : ""}`} onClick={() => setFilter(g)}>{g}</button>
      ))}
      <button type="button" className="chip wt-works-sort" onClick={() => setNewest((v) => !v)}
              aria-label={newest ? "최신순 — 누르면 오래된순" : "오래된순 — 누르면 최신순"}>
        {newest ? "최신순" : "오래된순"} <IconChevronDown size={14} />
      </button>
    </div>
  );
  ...
  const exampleToggle = hasExamples && (
    <label className="wt-works-extoggle">
      <input type="checkbox" checked={showExamples} aria-label="예시 작품 보기"
  ...
  return (
    <div className="wt-works">
      <MobileTop title="둘러보기" />
      <div className="wt-wrap wt-page wt-works-page">
        <div className="wt-works-head">
          <div className="wt-works-title">
            <h2>다른 사람들의 웹툰</h2>
            <span className="muted">표지를 누르면 그대로 읽을 수 있어요.</span>
          </div>
          <div className="wt-works-headacts">
            {exampleToggle}
            <button type="button" className="btn btn-p wt-works-create" onClick={() => go("entry")}>내 웹툰 만들기</button>
          </div>
        </div>
        ...
        {failed && (
          <div className="wt-works-state">
            ...
            <h2>목록을 가져오지 못했어요</h2>
            <span className="muted">서버에 닿지 못했어요. 잠시 뒤 다시 시도해 주세요 — 만들어 둔 작품은 그대로 있어요.</span>
            <div className="wt-works-stateacts">
              <button type="button" className="btn btn-p" onClick={() => setTick((n) => n + 1)}>다시 시도</button>
              <button type="button" className="btn btn-w" onClick={() => go("landing")}>홈으로</button>
            </div>
          </div>
        )}
        ...
        {runs && noneAtAll && (
          <div className="wt-works-state">
            ...
            <h2>아직 구경할 웹툰이 없어요</h2>
            <span className="muted">첫 작품이 이 자리에 걸립니다. 캐릭터 하나와 이야기 한 줄이면 10분 안에 한 편이 나와요.</span>
            <div className="wt-works-stateacts">
              <button type="button" className="btn btn-p" onClick={() => go("entry")}>내 캐릭터로 웹툰 만들기</button>
            </div>
          </div>
        )}
        ...
        {runs && !noneAtAll && hiddenByToggle && (
          <div className="wt-works-state">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={louArt("empty")} alt="" style={{ width: 180 }} />
            <h2>예시를 빼니 볼 게 없어요</h2>
            <span className="muted">작품이 없는 게 아니라 「예시 작품도 보기」를 꺼 둔 탓이에요. 실제 작품이 걸리면 이 자리에 먼저 보입니다.</span>
            <div className="wt-works-stateacts">
              <button type="button" className="btn btn-p" onClick={() => toggleExamples(true)}>예시 작품 다시 보기</button>
              <button type="button" className="btn btn-w" onClick={() => go("entry")}>내 웹툰 만들기</button>
            </div>
          </div>
        )}
...
function WorkCard({ run, mine, go }: { run: RunCard; mine: boolean; go: Go }) {
  ...
  const sub = [run.character, ...new Set([run.genre, run.style_label].filter(Boolean))].filter(Boolean).join(" · ");
  ...
  const flip = async () => {
    ...
    try {
      ...
      setPub(!want);
      setErr((e as Error).message || "바꾸지 못했어요");
    }
    setBusy(false);
  };
  ...
  return (
    <div className="card wt-works-card">
      <button type="button" className="wt-works-cover" onClick={open} aria-label={`${run.title || run.run_id} 열기`}>
        {run.example && <span className="badge wt-works-badge">예시</span>}
        {run.cover_page ? (
          /* eslint-disable-next-line @next/next/no-img-element */
      ...
      {eps.length > 0 && (
        <div className="wt-works-eps">
          {eps.map((n) => (
            <button key={n} type="button" className="ep" onClick={open}>{n}화</button>
          ))}
        </div>
      )}
      {mine && (
        <div className="wt-works-mine">
          <span className="wt-works-pub">
            <button type="button" className={`sw${pub ? "" : " off"}`} role="switch" aria-checked={pub}
                    aria-label="둘러보기에 공개" disabled={busy} onClick={flip}><i /></button>
            {pub ? "공개" : "비공개"}
          </span>
          <button type="button" className="wt-works-edit" onClick={() => go("editor", { run: run.run_id })}>편집실</button>
        </div>
      )}
