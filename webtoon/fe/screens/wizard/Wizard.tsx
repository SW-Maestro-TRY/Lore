...
function Crumb({ at }: { at: number }) {
  return (
    <div className="crumb wt-wiz-crumb" aria-label="지금 위치">
      {CRUMB.map((it, i) => (
...
export default function Wizard({
  ...
  const loadChars = () => {
    ...
    listCharacters()
      ...
      .catch((e: Error) => { setChars([]); setCharsErr(e.message || "캐릭터를 불러오지 못했습니다"); });
  };
  ...
  const addFiles = async (files: FileList | null) => {
    ...
    if (room <= 0) { setPhotoErr(`사진은 ${MAX_PHOTOS}장까지 올릴 수 있어요`); return; }
    const added: string[] = [];
    for (const f of list.slice(0, room)) {
      try { added.push(await readPhoto(f)); } catch (e) { setPhotoErr(e instanceof Error ? e.message : "사진을 열지 못했습니다"); }
    }
  ...
  const start = async () => {
    ...
    try {
      ...
      setStartErr(e instanceof Error ? e.message : "만들기를 시작하지 못했습니다");
      setStarting(false);
    }
  };
  ...
  return (
    <div className="wt-wiz">
      <MobileTop back={mBack} title={M_TITLE[step - 1]} right={`${step} / 6`} />
        ...
        {step === 1 && (
          <>
            <div className="wt-wiz-body">
              <div className="wt-wiz-left">
                {chars === null || (presetCharacterId && !presetChar) ? (
                  <>
                    <div className="wt-wiz-head">
                      <h2>누가 주인공인가요?</h2>
                      <span className="muted lede">만들어 둔 캐릭터를 고르거나, 사진을 올리세요. 아는 만큼만 적으면 돼요.</span>
                    </div>
                  ...
                  /* 골라서 들어왔을 때 */
                  <>
                    <div className="wt-wiz-head"><h2>{picked.name}과 함께 갈게요!</h2></div>
                    <div className="wt-wiz-picked">
                      <div className="ccard on">
                        <button type="button" className="wt-wiz-unpick" aria-label="다른 캐릭터로" title="다른 캐릭터로" onClick={unpick}>
                          <IconClose size={14} />
                        </button>
                      ...
                      <button type="button" className="btn btn-w" onClick={() => go("create", { step: 1 })}>다른 캐릭터 고르기</button>
                    </div>
                  </>
                ) : mine.length > 0 ? (
                  /* 내 캐릭터가 있을 때 */
                  <>
                    <div className="wt-wiz-head">
                      ...
                      <span className="muted lede">
                        ...
                        <button type="button" onClick={() => go("characters")}>내 캐릭터 전부 보기 <IconArrow size={14} /></button>
                      </span>
                    </div>
                    <div className="wt-wiz-cards">
                      {mine.map((c) => {
                        ...
                        return (
                          <div key={c.id} className={`ccard${on ? " on" : ""}`} role="button" tabIndex={0}
                            ...
                            {on && (
                              <>
                                <span className="wt-wiz-unpick" title="다른 캐릭터로" aria-hidden="true"><IconClose size={12} /></span>
                                <CheckMark />
                              </>
                            )}
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                    ...
                    <div className="wt-wiz-or"><i /><span className="dim">또는 캐릭터를 어떻게 넣을까요?</span><i /></div>
                    <Ways form={form} onUpload={() => fileRef.current?.click()} onRemove={(i) => patch({ photos: form.photos.filter((_, k) => k !== i) })}
                          makeLabel="캐릭터 직접 만들기" makeSub="사진이 없어도 설명만으로 그려요" onMake={() => go("try")} />
                  </>
                ) : (
                  /* 만든 것이 없을 때 */
                  <>
                    <div className="wt-wiz-head">
                      <h2>누가 주인공인가요?</h2>
                      <span className="muted lede">아직 만들어 둔 캐릭터가 없어요. 사진을 올리거나 하나 만들어 보세요.</span>
                    </div>
                    <Ways tall form={form} onUpload={() => fileRef.current?.click()} onRemove={(i) => patch({ photos: form.photos.filter((_, k) => k !== i) })}
                          makeLabel="캐릭터 만들어보기" makeSub="사진·설명 없이도 돼요" onMake={() => go("try")} />
                    {builtins.length > 0 && (
                      <>
                        <div className="wt-wiz-or"><i /><span className="dim">바로 써 볼 수 있는 캐릭터</span><i /></div>
                          ...
                          {builtins.map((c) => {
                            ...
                            return (
                              <div key={c.id} className={`ccard${on ? " on" : ""}`} role="button" tabIndex={0}
                                ...
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={c.art_url || ""} alt="" />
                                <div className="row"><b>{c.name}</b><span className="dim">둘러보기용</span></div>
                                <span className="muted">{c.card?.twist || c.description}</span>
                              </div>
                            );
                          })}
                ...
                {charsErr && (
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    <button type="button" className="btn btn-w btn-sm" onClick={loadChars}>다시 시도</button>
                  </div>
                )}
              ...
              <div className="wt-wiz-right">
                <div className="fieldset">
                  <label htmlFor="wt-wiz-nm">이름 <span className="wt-wiz-req">필수</span></label>
                  <input id="wt-wiz-nm" className="field" value={form.name} placeholder="예: 민시하" aria-label="이름"
                         onChange={(e) => patch({ name: e.target.value })} />
                  {form.characterId && mine.length > 0 && !presetCharacterId && (
                    <span className="dim" style={{ fontSize: 12 }}>고른 캐릭터에서 채워졌어요 · 사진을 골랐다면 직접 적어요</span>
                  )}
                </div>
                <div className="fieldset">
                  <label htmlFor="wt-wiz-ds">캐릭터 설명 <span className="dim wt-wiz-opt">선택</span></label>
                  <textarea id="wt-wiz-ds" className="field wt-wiz-desc" value={form.character} aria-label="캐릭터 설명"
                            placeholder="성격·말투·관계 등 아는 만큼. 예) 장난기 많은데 겁은 많아서 친구 앞에서만 센 척한다"
            ...
            <div className="wt-wiz-foot">
              {presetCharacterId ? (
                <button type="button" className="btn btn-w" onClick={() => go("characters")}><IconBack size={16} /> 이전 <span className="dim">· 내 캐릭터</span></button>
              ) : (
                <button type="button" className="btn btn-w" onClick={() => go("entry")}><IconBack size={16} /> 이전 <span className="dim">· 입구</span></button>
              )}
              <span className="dim mid">
                {presetCharacterId ? "1 / 4 · 다음은 이야기 · 장르" : "1 / 4 · 다음은 이야기 · 장르 · 사진(또는 캐릭터)과 이름이 있어야 넘어가요"}
              </span>
              <button type="button" className="btn btn-p" disabled={!step1Ok} onClick={() => goStep(2)}>다음 <IconArrow size={18} /></button>
            </div>
            <div className="mfoot">
              <button type="button" className="btn btn-p" disabled={!step1Ok} onClick={() => goStep(2)}>다음</button>
            </div>
          </>
        )}
          ...
          <>
            <div className="wt-wiz-body gap48">
              <div className="wt-wiz-story">
                <div className="wt-wiz-head">
                  <h2>어떤 이야기를 볼까요?</h2>
                  <span className="muted lede">한 줄이어도 되고 줄거리여도 돼요. 비우면 캐릭터를 보고 지어요.</span>
                </div>
                <textarea className="field wt-wiz-storybox" aria-label="이야기" value={form.story}
                          onChange={(e) => patch({ story: e.target.value })} />
              </div>
              <div className="wt-wiz-genre">
                <div className="wt-wiz-head">
                  <h2>장르</h2>
                  <span className="muted lede">안 고르면 이야기에 맞춰 정해요.</span>
                </div>
                <div className="wt-wiz-chips">
                  {GENRE_QUICK.map((g) => (
                    <button key={g} type="button" className={`chip${form.genre === g ? " on" : ""}`}
                            onClick={() => patch({ genre: form.genre === g ? "" : g })}>{g}</button>
                  ))}
                </div>
                <input className="field" value={genreCustom} placeholder="목록에 없으면 직접 적기 · 예: 무협 / 로맨스 판타지" aria-label="장르 직접 입력"
                       onChange={(e) => patch({ genre: e.target.value })} />
                {(!form.genre || GENRE_NOTE[form.genre]) && (
                  <div className="wt-wiz-gnote">{form.genre ? GENRE_NOTE[form.genre] : GENRE_NOTE_EMPTY}</div>
                )}
              </div>
            </div>
            <div className="wt-wiz-foot">
              <button type="button" className="btn btn-w" onClick={() => goStep(1)}><IconBack size={16} /> 이전 <span className="dim">· 캐릭터</span></button>
              <span className="dim mid">2 / 4 · 다음은 그림체 · 둘 다 비워도 만들 수 있어요</span>
              <button type="button" className="btn btn-p" onClick={() => goStep(3)}>다음 <IconArrow size={18} /></button>
            </div>
            <div className="mfoot">
              <span className="dim wt-wiz-mfoot-note">둘 다 비워도 만들 수 있어요</span>
              <button type="button" className="btn btn-p" onClick={() => goStep(3)}>다음</button>
            </div>
          </>
        )}
        ...
        {step === 3 && (
          <>
            <div className="wt-wiz-stylehead">
              <div className="wt-wiz-head">
                <h2>어떤 그림체로 그릴까요?</h2>
                <span className="muted lede">그림을 눌러 고르세요. 캐릭터 시트도 이 그림체로 그려져요.</span>
              </div>
              <span className="dim" style={{ fontSize: 12.5 }}>예시는 실제로 만들어진 편에서</span>
            </div>
            <div className="wt-wiz-styles">
              {STYLE_INFO.map(([key, label, desc]) => (
                <button key={key} type="button" className={`wt-wiz-style${form.style === key ? " on" : ""}`}
                  ...
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={STYLE_THUMB[key] || `/static/samples/ex-${key}-1.jpg`} alt={`${label} 예시`} />
                  <div className="txt"><b>{label}</b><span className="muted">{desc}</span></div>
                </button>
              ))}
            </div>
            <div className="wt-wiz-foot">
              <button type="button" className="btn btn-w" onClick={() => goStep(2)}><IconBack size={16} /> 이전 <span className="dim">· 이야기 · 장르</span></button>
              <span className="dim mid">3 / 4 · 다음은 방식</span>
              <button type="button" className="btn btn-p" disabled={!form.style} onClick={() => goStep(4)}>다음 <IconArrow size={18} /></button>
            </div>
            <div className="mfoot">
              <button type="button" className="btn btn-p" disabled={!form.style} onClick={() => goStep(4)}>다음</button>
            </div>
          </>
        )}
            ...
            <div className="wt-wiz-body gap48" style={{ marginTop: 14 }}>
                    ...
                    {QUALITY_INFO.map((q) => {
                      ...
                      return (
                        <button key={q.key} type="button" className={`wt-wiz-q${form.quality === q.key ? " on" : ""}`}
                          ...
                          <div className="row">
                            <b>{q.label}</b>
                            <i>{c == null ? <span className="skeleton" style={{ display: "inline-block", width: 48, height: 14, borderRadius: 4 }} /> : `${c}크레딧`}</i>
                          </div>
                          <span className="lede">{q.lede}</span>
                          <span className="muted desc">{q.desc.join(" ")}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div>
                  ...
                  <div className="wt-wiz-modes">
                    <button type="button" className={`wt-wiz-mode${form.mode === "simple" ? " on" : ""}`} onClick={() => patch({ mode: "simple" })}>
                      <b>빠르게 결과부터</b><span className="muted">중간에 안 멈추고 알아서 그려 와요.</span>
                    </button>
                    <button type="button" className={`wt-wiz-mode${form.mode === "expert" ? " on" : ""}`} onClick={() => patch({ mode: "expert" })}>
                      <b>2번 확인하며</b><span className="muted">캐릭터 시트와 이야기에서 한 번씩 멈춰 확인해요.</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="wt-wiz-side">
                <div className="wt-wiz-sum">
                  <b>이렇게 만들어요</b>
                  <div className="kvr"><span className="dim">캐릭터</span><span>{form.name.trim() || "—"}{form.characterId ? " · 내 캐릭터" : form.photos.length ? ` · 사진 ${form.photos.length}장` : ""}</span></div>
                  <div className="kvr"><span className="dim">이야기</span><span>{form.story.trim() ? form.story.trim() : "비움"}</span></div>
                  <div className="kvr"><span className="dim">장르</span><span>{form.genre.trim() || "비움"}</span></div>
                  <div className="kvr"><span className="dim">그림체</span><span>{styleLabel || "—"}</span></div>
                  <div className="kvr"><span className="dim">촘촘함</span><span>{quality.label} · {quality.lede.split(" · ")[0]}</span></div>
                  <div className="kvr"><span className="dim">보는 방식</span><span>{form.mode === "expert" ? "2번 확인하며" : "빠르게 결과부터"}</span></div>
                  <div className="tot">
                    <b>쓰는 크레딧</b>
                    <b>{allow && !allow.logged_in ? "무료" : cost == null ? "…" : cost}</b>
                  </div>
                </div>
                <label className="wt-wiz-agree">
                  <input type="checkbox" checked={form.agreeIp} aria-label="저작권 확인" onChange={(e) => patch({ agreeIp: e.target.checked })} />
                  <span>업로드한 사진·설정에 대한 저작권 문제가 없음을 확인합니다 <i>필수</i></span>
                </label>
                ...
                <button type="button" className="btn btn-p" disabled={!canStart} onClick={() => void start()}>
                  {starting ? <><span className="spin" /> 시작하는 중</> : "웹툰 만들기"}
                </button>
                <span className="dim hint">시간은 줄이 비었을 때 기준이에요. 앞에 사람이 있으면 더 걸려요.</span>
              </div>
            </div>
            <div className="wt-wiz-foot plain" style={{ justifyContent: "flex-start", paddingTop: 16 }}>
              <button type="button" className="btn-ghost" onClick={() => goStep(3)}><IconBack size={16} /> 그림체</button>
            </div>
            <div className="mfoot">
              <span className="dim wt-wiz-mfoot-note">{quality.lede.split(" · ")[0]} · 앞에 사람이 있으면 더 걸려요</span>
              <button type="button" className="btn btn-p" disabled={!canStart} onClick={() => void start()}>
                {starting ? <><span className="spin" /> 시작하는 중</> : "웹툰 만들기"}
              </button>
            </div>
  ...
  return (
    <div className={`wt-wiz-ways${tall ? " tall" : ""}`}>
      <div className={`wt-wiz-way${has ? " has" : ""}`} role="button" tabIndex={0} onClick={onUpload}
          ...
          <div className="wt-wiz-thumbs">
            {form.photos.map((p, i) => (
              <span key={i} className="wt-wiz-thumb">
                ...
                <button type="button" aria-label="사진 빼기" onClick={(e) => { e.stopPropagation(); onRemove(i); }}><IconClose size={11} /></button>
              </span>
            ))}
          </div>
        ) : (
        ...
        <b>캐릭터 사진 올리기</b>
        <span className="dim">{has ? `${form.photos.length} / ${MAX_PHOTOS}장 · 각도가 다양할수록 더 닮아요` : "최대 4장 · 각도가 다양할수록 더 닮아요"}</span>
      </div>
